# Windows brand breadcrumbs — monopin 1.1.0

**Audience:** Windows build machine operator. This is the **full product map**
for Suite **1.1.0**: client architecture, residual fleet, **Ned / rpAI / oracle**,
...
**Catalog monopin:** `1.1.0` (`client/VERSION` must match).

**Target Suite PE (must match basename):**

```text
releases/1.1.0/restore-privacy-client-1.1.0-windows-x64-setup.exe
```

Honesty: Mac may stage a **carry-forward** PE under that filename until this
machine rebuilds and Authenticode-seals a native **1.1.0** installer. Do not
...
**1.1.0 ship notes (this monopin):**

| Topic | 1.1.0 product truth |
|-------|---------------------|
...
| Free Suite download | Five catalog basenames under Helsinki `paid_assets/1.1.0/` |
...
Client Suite (this PE / Flutter / native)
  ...
  └── Settings: manual Suite update (opt-in catalog updates) — never force-install
...
| **Rx browser** | `restore-privacy-rx-browser-1.1.0-windows.zip` (optional) |
...
1. Sync monorepo; confirm `client/VERSION` = **`1.1.0`**.  
...
5. Output name: `restore-privacy-client-1.1.0-windows-x64-setup.exe`.  
6. Authenticode-sign.  
7. Stage → `status_page/assets/1.1.0/` and `releases/1.1.0/`.  
...
10. Breadcrumbs: `python3 scripts/breadcrumbs_vault.py stage --version 1.1.0` (+ publish).  
...
- Paid installers: Helsinki `paid_assets/1.1.0/` (token-gated).  
...
- `restore-privacy-rx-browser-1.1.0-windows.zip` if Rx browser ships with this monopin.  
- Brand-wide slots (rpOS, Pens, node-installer, …) via brand mirror — see §8.

## Honest status

- Catalog can serve monopin **1.1.0** with a re-pinned carry-forward PE until
