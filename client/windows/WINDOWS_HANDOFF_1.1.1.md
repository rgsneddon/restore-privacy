# Windows brand breadcrumbs — monopin 1.1.1

**Audience:** Windows build machine operator. This is the **full product map**
for Suite **1.1.1**: client architecture, residual fleet, **Ned / rpAI / oracle**,
...
**Catalog monopin:** `1.1.1` (`client/VERSION` must match).

**Target Suite PE (must match basename):**

```text
releases/1.1.1/restore-privacy-client-1.1.1-windows-x64-setup.exe
```

Honesty: Mac may stage a **carry-forward** PE under that filename until this
machine rebuilds and Authenticode-seals a native **1.1.1** installer. Do not
...
**1.1.1 ship notes (this monopin):**

| Topic | 1.1.1 product truth |
|-------|---------------------|
...
| Free Suite download | Five catalog basenames under Helsinki `paid_assets/1.1.1/` |
| Windows PE | Carry-forward under 1.1.1 basename until **native Authenticode seal on this machine** |
| Linux tarball | Carry-forward under 1.1.1 until native rebuild here |
...
| **Rx browser** | `restore-privacy-rx-browser-1.1.1-windows.zip` (optional) |
...
1. Sync monorepo; confirm `client/VERSION` = **`1.1.1`**.  
...
5. Output name: `restore-privacy-client-1.1.1-windows-x64-setup.exe`.  
6. Authenticode-sign.  
...
- `restore-privacy-rx-browser-1.1.1-windows.zip` if Rx browser ships with this monopin.  
- Brand-wide slots (rpOS, Pens, node-installer, …) via brand mirror — see §8.

## Honest status

- Catalog can serve monopin **1.1.1** with a re-pinned carry-forward PE until
