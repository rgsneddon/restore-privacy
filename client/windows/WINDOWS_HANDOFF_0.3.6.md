# Windows handoff — Restore Privacy **0.3.6** (multi-hop residual)
...
| Repo at **0.3.6** / `main` | `git checkout 0.3.6` or latest `main` with handoff |
...
5. Writes `releases/0.3.6/…-windows-x64-setup.exe` and refreshes `SHA256SUMS.json` / `manifest.json` when present

Recipe core: `scripts/build_release_0.0.8.py` (version constants overridden for 0.3.6).
...
```bat
python scripts\build_windows_multihop.py --check-only
dir releases\0.3.6\restore-privacy-client-0.3.6-windows-x64-setup.exe
```

Optional: search the binary (PowerShell) for multihop markers:

```powershell
Select-String -Path releases\0.3.6\restore-privacy-client-0.3.6-windows-x64-setup.exe -Pattern "exit_node_elgamal","185.146.232.107","multihop" -Encoding byte -ErrorAction SilentlyContinue
...
```bat
gh release upload 0.3.6 releases\0.3.6\restore-privacy-client-0.3.6-windows-x64-setup.exe --clobber
...
staged `status_page/assets/0.3.6/` + VPS paid_assets when configured.
...
| Log | Path |
|-----|------|
| Client freeze | `dist/0.3.6/pyinstaller_client.log` |
| Installer freeze | `dist/0.3.6/pyinstaller_installer.log` |
