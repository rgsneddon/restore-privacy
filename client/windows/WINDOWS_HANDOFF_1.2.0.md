...
**Do not** rename an ancient PE (e.g. payload `RestorePrivacy-0.5.8.exe`, tray `rpT0`, or monopin 1.1.5/1.1.6 carry) and claim 1.2.0 seal.
...
| Catalog pin | `status_page\downloads.py` → `RELEASE_VERSION = "1.2.0"` |
...
If pin is not 1.2.0, **stop** — wrong branch or stale pull.
...
6. Filename is exactly `restore-privacy-client-1.2.0-windows-x64-setup.exe`.

### Step 7 — Stage and upload to Helsinki paid assets

```bat
python scripts\host_paid_assets_vps.py --stage --upload --version 1.2.0 --force
```

Confirm remote:

```text
/opt/restore-privacy/paid_assets/1.2.0/restore-privacy-client-1.2.0-windows-x64-setup.exe
```

Also mirror under `status_page\assets\1.2.0\` if the script does not already.

### Step 8 — Re-publish breadcrumbs (so the next fetch sees seal status)

```bat
python scripts\breadcrumbs_vault.py stage --version 1.2.0
...
rem Output: releases\1.2.0\restore-privacy-client-1.2.0-linux-x64.tar.gz
```

Confirm desktop Name in packaged `install.sh` is exactly **`Privacy, Restored`**.  
Then upload with the same host paid-assets command (version 1.2.0).

**Arch Linux:**

```bat
python scripts\package_arch_linux.py
rem or staged tree under releases\1.2.0\arch
```

PKGBUILD / package monopin must read **1.2.0** (not a stale 1.1.6 carry rename).  
...
| Platform | Mac agent status (1.2.0) |
|----------|---------------------------|
...
| Symptom | Fix |
|---------|-----|
| Pin not 1.2.0 after pull | Wrong branch; `git checkout main && git pull` |
| PE still named 1.1.6 | Rebuild with `--version 1.2.0`; do not rename |
