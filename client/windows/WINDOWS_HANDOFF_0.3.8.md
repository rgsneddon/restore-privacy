# Windows handoff — Restore Privacy **0.3.8** (multi-hop residual)
...
Catalog **0.3.8** Windows multihop residual PE is built on a **Windows x64**
machine via the one-command path below (macOS cannot freeze a Windows PE).
Paid asset name: `restore-privacy-client-0.3.8-windows-x64-setup.exe`.
...
| Repo at **0.3.8** / `main` | `git checkout 0.3.8` or latest `main` with handoff |
...
5. Writes `releases/0.3.8/…-windows-x64-setup.exe` and refreshes `SHA256SUMS.json` / `manifest.json` when present

Recipe core: `scripts/build_release_0.0.8.py` (version constants overridden for 0.3.8).
...
```bat
python scripts\build_windows_multihop.py --check-only
dir releases\0.3.8\restore-privacy-client-0.3.8-windows-x64-setup.exe
```

Optional: search the binary (PowerShell) for multihop markers:

```powershell
Select-String -Path releases\0.3.8\restore-privacy-client-0.3.8-windows-x64-setup.exe -Pattern "exit_node_elgamal","185.146.232.107","multihop" -Encoding byte -ErrorAction SilentlyContinue
...
```bat
gh release upload 0.3.8 releases\0.3.8\restore-privacy-client-0.3.8-windows-x64-setup.exe --clobber
...
staged `status_page/assets/0.3.8/` + VPS paid_assets when configured.
...
| Log | Path |
|-----|------|
| Client freeze | `dist/0.3.8/pyinstaller_client.log` |
| Installer freeze | `dist/0.3.8/pyinstaller_installer.log` |
...
- Catalog / release notes: `scripts/RELEASE_NOTES_0.3.8.md`
- Full multi-platform release script: `scripts/build_release_0.3.8.py` (`--windows-only` rebuilds Windows via this path)
- Multihop unit tests: `tests/test_multihop.py`
- Package pin honesty: `tests/test_release_0_3_6_package_pins.py` (Windows PE multihop gates run when the rebuilt setup is present under `releases/0.3.8/`)
