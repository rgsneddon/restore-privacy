# Windows brand breadcrumbs — monopin 1.1.5

**Audience:** Windows x64 build machine operator (and Linux/Arch rebuild when done here).  
**You must native-rebuild the Windows PE for 1.1.5.** Mac agents only stage carry-forward EXE; that is **not** a seal.
...
**Catalog monopin:** `1.1.5` — must match `client\VERSION`.
...
**Do not** rename an ancient PE (e.g. payload `RestorePrivacy-0.5.8.exe`, tray `rpT0`, or monopin 1.1.3/1.1.4 carry) and claim 1.1.5 seal.

---

## 0. Product truth that MUST be in this PE (1.1.5)
...
| Catalog pin | `status_page\downloads.py` → `RELEASE_VERSION = "1.1.5"` |
...
If pin is not 1.1.5, **stop** — wrong branch or stale pull.
...
5. Filename is exactly `restore-privacy-client-1.1.5-windows-x64-setup.exe`.

### Step 7 — Stage and upload to Helsinki paid assets

```bat
python scripts\host_paid_assets_vps.py --stage --upload --version 1.1.5 --force
```

Confirm remote:

```text
/opt/restore-privacy/paid_assets/1.1.5/restore-privacy-client-1.1.5-windows-x64-setup.exe
```

Also mirror under `status_page\assets\1.1.5\` if the script does not already.

### Step 8 — Re-publish breadcrumbs (so the next fetch sees seal status)

```bat
python scripts\breadcrumbs_vault.py stage --version 1.1.5
...
rem Output: releases\1.1.5\restore-privacy-client-1.1.5-linux-x64.tar.gz
```

Confirm desktop Name in packaged `install.sh` is exactly **`Privacy, Restored`**.  
Then upload with the same host paid-assets command (version 1.1.5).

Arch: `scripts\package_arch_linux.py` or staged tree under `releases\1.1.5\arch`.

---

## 3. What Mac already did (do not re-do as PE)

| Platform | Mac agent status (1.1.5) |
|----------|---------------------------|
...
| Linux | `package_linux.py` rebuild with VERSION 1.1.5 + Name=Privacy, Restored |
...
| Mistake | Result |
|---------|--------|
| Rename old setup.exe to `…1.1.5…` | Catalog lies; tray/Quit/first-run wrong |
...
```text
[ ] git pull main; client\VERSION == 1.1.5
[ ] tray_win.py TRAY_DISPLAY_NAME == "Privacy, Restored"
[ ] build_windows_multihop.py --check-only --version 1.1.5  → OK
[ ] build_windows_multihop.py --version 1.1.5  → setup.exe created
[ ] Authenticode-sign setup.exe
[ ] Verify tray string + Quit lower-left + no account/seed first-run
[ ] host_paid_assets_vps.py --stage --upload --version 1.1.5 --force
[ ] breadcrumbs_vault.py stage + publish (optional but preferred)
[ ] Confirm paid_assets/1.1.5 Windows basename on Helsinki
```

**You are the seal.** Until these boxes are done, 1.1.5 Windows free-DL is not product-complete.
