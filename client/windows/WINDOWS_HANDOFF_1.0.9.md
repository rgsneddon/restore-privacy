# Windows brand breadcrumbs — monopin 1.0.9

**Audience:** Windows build machine operator. This is the **full product map**
for Suite **1.0.9**: client architecture, residual fleet, **Ned / rpAI / oracle**,
...
**Catalog monopin:** `1.0.9` (`client/VERSION` must match).

**Target Suite PE (must match basename):**

```text
releases/1.0.9/restore-privacy-client-1.0.9-windows-x64-setup.exe
```

Honesty: Mac may stage a **carry-forward** PE under that filename until this
machine rebuilds and Authenticode-seals a native **1.0.9** installer. Do not
...
**1.0.9 ship notes (this monopin):**

| Topic | 1.0.9 product truth |
|-------|---------------------|
...
| **Rx browser** | `restore-privacy-rx-browser-1.0.9-windows.zip` (optional) |
...
1. Sync monorepo; confirm `client/VERSION` = **`1.0.9`**.  
...
5. Output name: `restore-privacy-client-1.0.9-windows-x64-setup.exe`.  
6. Authenticode-sign.  
7. Stage → `status_page/assets/1.0.9/` and `releases/1.0.9/`.  
...
10. Breadcrumbs: `python3 scripts/breadcrumbs_vault.py stage --version 1.0.9` (+ publish).  
...
- Paid installers: Helsinki `paid_assets/1.0.9/` (token-gated).  
...
- `restore-privacy-rx-browser-1.0.9-windows.zip` if Rx browser ships with this monopin.  
- Brand-wide slots (rpOS, Pens, node-installer, …) via brand mirror — see §8.

## Honest status

- Catalog can serve monopin **1.0.9** with a re-pinned carry-forward PE until
