# Windows handoff â€” catalog **0.4.4**
...
## Pin
1. Pins `client/VERSION` â†’ `0.4.4`
2. Catalog `status_page/downloads.py` `RELEASE_VERSION` / `RELEASE_TAG` â†’ `0.4.4`
3. Filename: `restore-privacy-client-0.4.4-windows-x64-setup.exe`

## Settings defaults (0.4.4)
...
rem or:
python scripts\build_release_0.4.4.py --windows-only
```

Then copy the setup into `releases/0.4.4/` and re-run assure / GH release asset upload.

## Residual privilege (no Run as admin every day)
Honest residual (Wintun + dual /1) **always needs OS privilege somewhere**.

Product UX (0.4.4+):
1. Desktop shortcut opens as a **standard user** (not â€œRun as administratorâ€).
2. **Connect** uses `connect_residual_privilege_dispatch()` when the GUI is
   non-admin â€” never falls through to `start_full_tunnel` as a standard user.
3. If the residual helper task is installed, Connect **always** runs
   ...
   means â€œthis window need not elevateâ€).
4. Without a helper, Connect prompts UAC once to re-open elevated with
   `--rpt-auto-connect`.
5. Settings â†’ **Install residual helper (one-time Administrator)** registers the
...
- Catalog 0.4.4 filename alone is not proof of native Windows code freeze.
