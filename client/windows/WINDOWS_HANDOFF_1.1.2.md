# Windows brand breadcrumbs — monopin 1.1.2

**Audience:** Windows build machine operator. This is the **full product map**
for Suite **1.1.2**: client architecture, residual fleet, **Ned / rpAI / oracle**,
...
**Catalog monopin:** `1.1.2` (`client/VERSION` must match).

**Target Suite PE (must match basename):**

```text
releases/1.1.2/restore-privacy-client-1.1.2-windows-x64-setup.exe
```

Honesty: Mac may stage a **carry-forward** PE under that filename until this
machine rebuilds and Authenticode-seals a native **1.1.2** installer. Do not
...
**1.1.2 ship notes (this monopin):**

| Topic | 1.1.2 product truth |
|-------|---------------------|
...
| Free Suite download | Five catalog basenames under Helsinki `paid_assets/1.1.2/` |
...
| Residual watchdog | Pure `evaluateResidualWatchdog` — capture/IPv6/DNS drop while Connected is not OK |
...
| Linux rebuild | On this Windows machine: rebuild `linux-x64.tar.gz` under monopin 1.1.2; Mac may stage carry-forward only |
| Windows PE | Carry-forward under 1.1.2 basename until **native Authenticode seal on this machine** |
| Linux tarball | Carry-forward under 1.1.2 until native rebuild here |
...
| **Rx browser** | `restore-privacy-rx-browser-1.1.2-windows.zip` (optional) |
...
1. Sync monorepo; confirm `client/VERSION` = **`1.1.2`**.  
...
5. Output name: `restore-privacy-client-1.1.2-windows-x64-setup.exe`.  
6. Authenticode-sign.  
7. Stage → `status_page/assets/1.1.2/` and `releases/1.1.2/`.  
...
10. Breadcrumbs: `python3 scripts/breadcrumbs_vault.py stage --version 1.1.2` (+ publish).  
...
- Paid installers: Helsinki `paid_assets/1.1.2/` (token-gated).  
...
- `restore-privacy-rx-browser-1.1.2-windows.zip` if Rx browser ships with this monopin.  
- Brand-wide slots (rpOS, Pens, node-installer, …) via brand mirror — see §8.

## Honest status

- Catalog can serve monopin **1.1.2** with a re-pinned carry-forward PE until
...
## Windows + Linux operator continue (1.1.2 leak-posture ship)
...
- Catalog monopin **1.1.2** packages staged; macOS DevID notarized when sealed here
- Helsinki `paid_assets/1.1.2/` + breadcrumbs vault current
...
2. Open this file + monorepo at monopin 1.1.2
3. Rebuild suite PE → `releases/1.1.2/restore-privacy-client-1.1.2-windows-x64-setup.exe`
4. Authenticode-sign; upload Helsinki paid_assets/1.1.2
5. Re-publish breadcrumbs; re-run free-DL probe for windows basename

### Linux rebuild (on Windows build host or Linux agent)
1. Rebuild `restore-privacy-client-1.1.2-linux-x64.tar.gz` from monorepo
