...
REM Kill-switch rollback + IPv6 restore (does not remove product RPT-FW allows)
...
REM Re-apply scoped product Defender Firewall allows for next Connect
if exist "%~dp0AllowFirewall.bat" call "%~dp0AllowFirewall.bat" /quiet

if "%QUIET%"=="1" exit /b 0
echo.
echo Internet residual path restored. Windows Defender allows re-applied for Connect.
