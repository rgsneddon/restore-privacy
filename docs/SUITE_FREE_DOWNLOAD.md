...
Restore Privacy Suite **v1.0.5** installers are **not** anonymous freebies.
...
**Catalog monopin:** `1.0.5` (`status_page/downloads.py` `RELEASE_VERSION`,
`client/VERSION`).
...
| Linux | `restore-privacy-client-1.0.5-linux-x64.tar.gz` |

Store path: `paid_assets/1.0.5/{filename}` on Helsinki `135.181.152.10`.

## Operators building packages

```bash
# Suite client monopin (Darwin: Flutter android/macos/ios; win/linux carry-forward or native agent)
python3 scripts/build_suite_1.0.5.py
...
python3 scripts/host_paid_assets_vps.py --stage --upload --version 1.0.5 --force --install-serve
...
- `client/windows/WINDOWS_HANDOFF_1.0.5.md`
- `releases/1.0.5/WINDOWS_BREADCRUMBS.md`
...
```text
restore-privacy-client-1.0.5-windows-x64-setup.exe
```
...
- macOS CFBundle / Windows embedded product version must match monopin **1.0.5**
