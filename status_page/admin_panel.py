...
ADMIN_ARCHITECTURE_BLURB = (
    "Suite installers are free on the shop; residual Connect needs a KEYGEN after "
    f"Stripe (Monthly {PRICE_LABEL} / Yearly {PRICE_YEARLY_LABEL} GBP). Catalog peers: "
    "Germany (default entry) and Singapore — multi-hop opt-in uses the other peer. "
    "US and Romania residual peers are retired. Weekly wipe is sequential IS → DE. "
    "Public status stays title-only. Licences and paid grants sit on the durable "
    "payment disk, not residual scratch."
)

# Human-cadence product architecture copy for the admin home main pane.
ADMIN_ARCHITECTURE_FULL = (
    "Customers start with a free Suite installer from the public shop (Windows, "
    "Android, macOS, iOS, Linux). Download alone does not open residual traffic: "
    "Connect waits for a KEYGEN from fulfilment after Stripe Checkout "
    f"(monthly {PRICE_LABEL} or yearly {PRICE_YEARLY_LABEL} GBP). The KEYGEN is "
    "what turns a working install into residual VPN use while the licence is OK. "
    "The Suite client is proprietary residual software — public Pages are not a "
    "full FOSS dump of the operator console.\n\n"
    "Residual entry is Germany by default, with Singapore as the other live peer. "
    "Users pick the country in the app. Multi-hop is optional and, when enabled, "
    "uses residual-via-exit through the other catalog peer (not full onion "
    "encapsulation as a product claim). United States and Romania residual hosts "
    "stay retired and are not public dial targets.\n\n"
    "rpOS is a desktop-only RESTORE path (Windows / macOS / Linux installers) with "
    "free Pens · Tables · Slides on the Desktop after install. There is no mobile "
    "rpOS install. rpOS instances can enable a light flyclient hidden multi-hop "
    "hook for participation — never listed as public residual entry/exit and not "
    "the full fleet selfhost/zram+LUKS stack. rpMail and rpOffice are brand "
    "desktop package slots; node-installer and node-operator packages are "
    "operator-facing. Browser and Vault public landings are schedule-only style "
    "pages (not the VPN shop structure).\n\n"
    "About once a week the operator path rebuilds residual peers one at a time "
    "(Germany, then Singapore) under an exclusive lock so both never wipe together. "
    "Clients can move to a healthy peer while one rebuilds. Public pages stay "
    "title-only — no live client count on the shop or node status.\n\n"
    "This admin surface covers payment readiness, one-off links and keygens under "
    "Link Generation, brand-wide Helsinki package push (Suite + rpOS + apps + "
    "operator slots), and durable licence history under Active Licences. That "
    "database should live on the attached disk so grants survive residual wipe "
    "and Render redeploy. It is not residual runtime scratch.\n\n"
    "When a paid period ends or a subscription is revoked, Connect stops until "
    "the customer renews and enters a valid KEYGEN. Ended rows remain visible so "
    "you can still audit history."
)
