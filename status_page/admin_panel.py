...
ADMIN_ARCHITECTURE_BLURB = (
    "Residual catalog peers: Germany (DE, default entry), Iceland (IS) only — "
    "user-selectable entry; multi-hop opt-in uses the other catalog peer. "
    "United States (US) and Romania (RO) residual peers are retired (prefs "
    "normalize to DE). Weekly fleet wipe is sequential IS → DE (exclusive "
...
ADMIN_ARCHITECTURE_FULL = (
    ...
    "second product they buy. United States (US) and Romania (RO) residual peers "
    "are retired and must not reappear as live pins.\n\n"
    "How the fleet is kept honest: about once a week the operator path wipes and "
    "rebuilds residual peers one at a time (Iceland → Germany), never both at once. "
    "Clients can hop to a healthy peer while one is rebuilding. "
