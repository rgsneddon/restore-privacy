...
| **Audit generated** | **12 August 2026** (`2026-08-12T06:00:21Z`) |
...
## Installer package AUDIT STATE (catalog v1.2.1)
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-1.2.1-windows-x64-setup.exe` | 🟧 | catalog monopin asset not staged (looked for restore-privacy-client-1.2.1-windows-x64-setup.exe / relative_path 1.2.1/restore-privacy-client-1.2.1-windows-x64-setup.exe under status_page/assets/1.2.1/, releases/1.2.1/, /opt/restore-privacy/paid_assets/1.2.1/, dist/1.2.1/, status_page/assets/, https://135.181.152.10.sslip.io/paid-assets/1.2.1/; default paid store is Helsinki https://135.181.152.10.sslip.io/paid-assets/ — set RPT_ASSET_FETCH_TOKEN to probe, or stage under paid_assets/1.2.1/); Windows PE pending native rebuild on Windows host (see client/windows/WINDOWS_HANDOFF_1.2.1.md; not seal-claimed — catalog ticker is Amber, not Red, while handoff is open) |
| 🤖 **Android** | `restore-privacy-client-1.2.1-android.apk` | 🟩 | node_elgamal.pub pin matches product monopin; Android residual wire (PFS + outer obfs) present; exit_node_elgamal.pub present (multihop exit pin) |
| 🍎 **macOS** | `restore-privacy-client-1.2.1-macos.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 📱 **iOS** | `restore-privacy-client-1.2.1-ios.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 🐧 **Linux** | `restore-privacy-client-1.2.1-linux-x64.tar.gz` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
...
- Entry probe: **54 ms** via `tcp` (shared base across rows).
...
|-------|------------|-----------|-------------------|------------------|-----|-----|-------|
| on | on | on | 54–59 ms (live base + shape feel) | 37–57 ms | 51.8 ms | 🟧 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT approximate UK band; AVG 51.8 ms → amber |
| on | on | off | 54–59 ms (live base + shape feel) | n/a (multi-hop off) | 56.5 ms | 🟧 | single-hop residual → entry; shape on; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; AVG 56.5 ms → amber |
| on | off | on | 54–59 ms (live base + shape feel) | 37–57 ms | 51.8 ms | 🟧 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT approximate UK band; AVG 51.8 ms → amber |
| on | off | off | 54–59 ms (live base + shape feel) | n/a (multi-hop off) | 56.5 ms | 🟧 | single-hop residual → entry; shape on; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; AVG 56.5 ms → amber |
| off | on | on | 54 ms (live) | 32–52 ms | 48 ms | 🟧 | residual dials exit when multi-hop on; shape off leaner; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT approximate UK band; AVG 48.0 ms → amber |
| off | on | off | 54 ms (live) | n/a (multi-hop off) | 54 ms | 🟧 | single-hop residual → entry; shape off (faster feel); outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; AVG 54.0 ms → amber |
| off | off | on | 54 ms (live) | 32–52 ms | 48 ms | 🟧 | residual dials exit when multi-hop on; shape off leaner; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT approximate UK band; AVG 48.0 ms → amber |
| off | off | off | 54 ms (live) | n/a (multi-hop off) | 54 ms | 🟧 | single-hop residual → entry; shape off (faster feel); outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; AVG 54.0 ms → amber |
...
**Overall posture:** **Strong** for residual honesty (`residual_ip_capture`), no public live count, no-phones-home Connect, packaging strip of `*.priv`, tunnel DNS + DoT, Settings transparency. Multi-hop residual is **opt-in** (`RPT_MULTIHOP_ENABLED=1`): residual-via-exit among non-entry catalog peers (exit **Germany (DE)**); default single-hop **Germany (DE)** entry — not full intermediate encapsulation. Catalog monopin **1.2.1**. Product kill-switch is **off by default** (opt-in ``RPT_KILL_SWITCH=1`` only). Installer package confidence is the RAG table at the top of this audit.
...
| Paid packages | `status_page/downloads.py` monopin **1.2.1**; assets under `status_page/assets/1.2.1/` + VPS `paid_assets` |
| Public web | `status_page/*` catalog **1.2.1** |
...
|----------|---------|--------|
| **Info** | Automated pass at `2026-08-12T06:00:21Z` | Recorded |
...
| Public catalog | **1.2.1** paid installers on [status host](https://restoreprivacy.online/) (£3.00/month or £30.00/year KEYGEN; free Suite installers via FREE DOWNLOAD; no free GitHub release downloads) |
...
| Code baseline | Catalog **1.2.1** + node **Germany (DE)** |
