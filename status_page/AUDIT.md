...
| **Audit generated** | **14 August 2026** (`2026-08-14T08:44:17Z`) |
...
## Installer package AUDIT STATE (catalog v1.2.4)
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-1.2.4-windows-x64-setup.exe` | 🟧 | catalog monopin asset not staged (looked for restore-privacy-client-1.2.4-windows-x64-setup.exe / relative_path 1.2.4/restore-privacy-client-1.2.4-windows-x64-setup.exe under status_page/assets/1.2.4/, releases/1.2.4/, /opt/restore-privacy/paid_assets/1.2.4/, dist/1.2.4/, status_page/assets/, https://135.181.152.10.sslip.io/paid-assets/1.2.4/; default paid store is Helsinki https://135.181.152.10.sslip.io/paid-assets/ — set RPT_ASSET_FETCH_TOKEN to probe, or stage under paid_assets/1.2.4/); Windows PE pending native rebuild on Windows host (see client/windows/WINDOWS_HANDOFF_1.2.4.md; not seal-claimed — catalog ticker is Amber, not Red, while handoff is open) |
| 🤖 **Android** | `restore-privacy-client-1.2.4-android.apk` | 🟩 | node_elgamal.pub pin matches product monopin; Android residual wire (PFS + outer obfs) present; exit_node_elgamal.pub present (multihop exit pin) |
| 🍎 **macOS** | `restore-privacy-client-1.2.4-macos.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 📱 **iOS** | `restore-privacy-client-1.2.4-ios.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 🐧 **Linux** | `restore-privacy-client-1.2.4-linux-x64.tar.gz` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
...
- Entry probe: **49 ms** via `tcp` (shared base across rows).
...
|-------|------------|-----------|-------------------|------------------|-----|-----|-------|
| on | on | on | 49–54 ms (live base + shape feel) | 37–57 ms | 49.2 ms | 🟧 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT approximate UK band; AVG 49.2 ms → amber |
| on | on | off | 49–54 ms (live base + shape feel) | n/a (multi-hop off) | 51.5 ms | 🟧 | single-hop residual → entry; shape on; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; AVG 51.5 ms → amber |
| on | off | on | 49–54 ms (live base + shape feel) | 37–57 ms | 49.2 ms | 🟧 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT approximate UK band; AVG 49.2 ms → amber |
| on | off | off | 49–54 ms (live base + shape feel) | n/a (multi-hop off) | 51.5 ms | 🟧 | single-hop residual → entry; shape on; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; AVG 51.5 ms → amber |
| off | on | on | 49 ms (live) | 32–52 ms | 45.5 ms | 🟧 | residual dials exit when multi-hop on; shape off leaner; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT approximate UK band; AVG 45.5 ms → amber |
| off | on | off | 49 ms (live) | n/a (multi-hop off) | 49 ms | 🟧 | single-hop residual → entry; shape off (faster feel); outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; AVG 49.0 ms → amber |
| off | off | on | 49 ms (live) | 32–52 ms | 45.5 ms | 🟧 | residual dials exit when multi-hop on; shape off leaner; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT approximate UK band; AVG 45.5 ms → amber |
| off | off | off | 49 ms (live) | n/a (multi-hop off) | 49 ms | 🟧 | single-hop residual → entry; shape off (faster feel); outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; AVG 49.0 ms → amber |
...
| Check | Result |
|-------|--------|
| Security unit suite | **PASS** (13 modules) |
...
**Overall posture:** **Strong** for residual honesty (`residual_ip_capture`), no public live count, no-phones-home Connect, packaging strip of `*.priv`, tunnel DNS + DoT, Settings transparency. Multi-hop residual is **opt-in** (`RPT_MULTIHOP_ENABLED=1`): residual-via-exit among non-entry catalog peers (exit **Germany (DE)**); default single-hop **Germany (DE)** entry — not full intermediate encapsulation. Catalog monopin **1.2.4**. Product kill-switch is **off by default** (opt-in ``RPT_KILL_SWITCH=1`` only). Installer package confidence is the RAG table at the top of this audit.
...
| Paid packages | `status_page/downloads.py` monopin **1.2.4**; assets under `status_page/assets/1.2.4/` + VPS `paid_assets` |
| Public web | `status_page/*` catalog **1.2.4** |
...
|----------|---------|--------|
| **Info** | Automated pass at `2026-08-14T08:44:17Z` | Recorded |
...
## 6. Automated checks (this pass — 14 August 2026)
...
| Public catalog | **1.2.4** paid installers on [status host](https://restoreprivacy.online/) (£3.00/month or £30.00/year KEYGEN; free Suite installers via FREE DOWNLOAD; no free GitHub release downloads) |
...
| Code baseline | Catalog **1.2.4** + node **Germany (DE)** |
| Pass date | **14 August 2026** |
