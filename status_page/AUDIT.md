...
## Installer package AUDIT STATE (catalog v1.0.1)
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-1.0.1-windows-x64-setup.exe` | 🟩 | present on Helsinki paid_assets store (https://135.181.152.10.sslip.io/paid-assets/1.0.0/restore-privacy-client-1.0.1-windows-x64-setup.exe; 38631642 bytes); structural pin/PE gates not re-scanned over HTTP (inventory presence from fulfilment host) |
| 🤖 **Android** | `restore-privacy-client-1.0.1-android.apk` | 🟩 | node_elgamal.pub pin matches product monopin; Android residual wire (PFS + outer obfs) present; exit_node_elgamal.pub present (multihop exit pin) |
| 🍎 **macOS** | `restore-privacy-client-1.0.1-macos.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 📱 **iOS** | `restore-privacy-client-1.0.1-ios.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 🐧 **Linux** | `restore-privacy-client-1.0.1-linux-x64.tar.gz` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
...
**Overall posture:** **Strong** for residual honesty (`residual_ip_capture`), no public live count, no-phones-home Connect, packaging strip of `*.priv`, tunnel DNS + DoT, Settings transparency. Multi-hop residual is **opt-in** (`RPT_MULTIHOP_ENABLED=1`): residual-via-exit among non-entry catalog peers (exit **Germany (DE)**); default single-hop **Germany (DE)** entry — not full intermediate encapsulation. Catalog monopin **1.0.1**. Product kill-switch is **off by default** (opt-in ``RPT_KILL_SWITCH=1`` only). Installer package confidence is the RAG table at the top of this audit.
...
| Paid packages | `status_page/downloads.py` monopin **1.0.1**; assets under `status_page/assets/1.0.0/` + VPS `paid_assets` |
| Public web | `status_page/*` catalog **1.0.1** |
...
| Public catalog | **1.0.1** Suite free installers on [status host](https://restoreprivacy.online/); KEYGEN from £3.00/month (VPN APP Shop) |
...
| Code baseline | Catalog **1.0.1** + node **Germany (DE)** |
