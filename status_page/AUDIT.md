...
## Installer package AUDIT STATE (catalog v1.0.6)
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-1.0.6-windows-x64-setup.exe` | 🟩 | present on Helsinki paid_assets store (https://135.181.152.10.sslip.io/paid-assets/1.0.0/restore-privacy-client-1.0.6-windows-x64-setup.exe; 38631642 bytes); structural pin/PE gates not re-scanned over HTTP (inventory presence from fulfilment host) |
| 🤖 **Android** | `restore-privacy-client-1.0.6-android.apk` | 🟩 | node_elgamal.pub pin matches product monopin; Android residual wire (PFS + outer obfs) present; exit_node_elgamal.pub present (multihop exit pin) |
| 🍎 **macOS** | `restore-privacy-client-1.0.6-macos.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 📱 **iOS** | `restore-privacy-client-1.0.6-ios.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 🐧 **Linux** | `restore-privacy-client-1.0.6-linux-x64.tar.gz` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
...
| Paid packages | `status_page/downloads.py` monopin **1.0.6**; assets under `status_page/assets/1.0.0/` + VPS `paid_assets` |
| Public web | `status_page/*` catalog **1.0.6** |
...
| Public catalog | **1.0.6** Suite free installers on [status host](https://restoreprivacy.online/); KEYGEN from £3.00/month (VPN APP Shop) |
...
| Code baseline | Catalog **1.0.6** + node **Germany (DE)** |
