...
| **Audit generated** | **4 August 2026** (`2026-08-04T21:22:07Z`) |
...
## Installer package AUDIT STATE (catalog v1.1.9)
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-1.1.9-windows-x64-setup.exe` | 🟥 | catalog monopin asset not staged (looked for restore-privacy-client-1.1.9-windows-x64-setup.exe / relative_path 1.1.9/restore-privacy-client-1.1.9-windows-x64-setup.exe under status_page/assets/1.1.9/, releases/1.1.9/, /opt/restore-privacy/paid_assets/1.1.9/, dist/1.1.9/, status_page/assets/, https://135.181.152.10.sslip.io/paid-assets/1.1.9/; default paid store is Helsinki https://135.181.152.10.sslip.io/paid-assets/ — set RPT_ASSET_FETCH_TOKEN to probe, or stage under paid_assets/1.1.9/) |
| 🤖 **Android** | `restore-privacy-client-1.1.9-android.apk` | 🟩 | node_elgamal.pub pin matches product monopin; Android residual wire (PFS + outer obfs) present; exit_node_elgamal.pub present (multihop exit pin) |
| 🍎 **macOS** | `restore-privacy-client-1.1.9-macos.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 📱 **iOS** | `restore-privacy-client-1.1.9-ios.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 🐧 **Linux** | `restore-privacy-client-1.1.9-linux-x64.tar.gz` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
...
- Exit probe: **93 ms** via `tcp` (shared base for multi-hop rows).
...
|-------|------------|-----------|-------------------|------------------|-----|-----|-------|
| on | on | on | 68–73 ms (live base + shape feel) | 93–98 ms (live base + shape feel) | 83 ms | 🟥 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 83.0 ms → red |
...
| on | off | on | 68–73 ms (live base + shape feel) | 93–98 ms (live base + shape feel) | 83 ms | 🟥 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 83.0 ms → red |
...
| off | on | on | 68 ms (live) | 93 ms (live) | 80.5 ms | 🟥 | residual dials exit when multi-hop on; shape off leaner; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 80.5 ms → red |
...
| off | off | on | 68 ms (live) | 93 ms (live) | 80.5 ms | 🟥 | residual dials exit when multi-hop on; shape off leaner; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 80.5 ms → red |
...
**Overall posture:** **Strong** for residual honesty (`residual_ip_capture`), no public live count, no-phones-home Connect, packaging strip of `*.priv`, tunnel DNS + DoT, Settings transparency. Multi-hop residual is **opt-in** (`RPT_MULTIHOP_ENABLED=1`): residual-via-exit among non-entry catalog peers (exit **Germany (DE)**); default single-hop **Germany (DE)** entry — not full intermediate encapsulation. Catalog monopin **1.1.9**. Product kill-switch is **off by default** (opt-in ``RPT_KILL_SWITCH=1`` only). Installer package confidence is the RAG table at the top of this audit.
...
| Paid packages | `status_page/downloads.py` monopin **1.1.9**; assets under `status_page/assets/1.1.9/` + VPS `paid_assets` |
| Public web | `status_page/*` catalog **1.1.9** |
...
|------|----------------------|----------------|-------------------|
| Iceland (IS) | ok=True err=None | code=200 body={'title': 'RESTORE PRIVACY'} | sent=True err=None |
...
|----------|---------|--------|
| **Info** | Automated pass at `2026-08-04T21:22:07Z` | Recorded |
...
| Public catalog | **1.1.9** paid installers on [status host](https://restoreprivacy.online/) (£3.00/month or £30.00/year KEYGEN; free Suite installers via FREE DOWNLOAD; no free GitHub release downloads) |
...
| Code baseline | Catalog **1.1.9** + node **Germany (DE)** |
| Pass date | **4 August 2026** |
| Machine JSON | `status_page/static/security_audit_latest.json` (when `--write`) |

## Client structural notes (catalog 1.1.9)

| Area | Honesty |
|------|---------|
| **First-use** | `AppEntryRoot` / `FirstRunPortal`: **Step 1 of 2** licence, **Step 2 of 2** KEYGEN or free trial. KEYGEN alone does not unlock without licence. |
| **Trial expired** | `assertMayConnect` surfaces `kTrialExpiredUnlockMsg` (purchase KEYGEN) — not Packet Tunnel copy — when trial ended and no active KEYGEN. |
