...
| **Audit generated** | **14 August 2026** (`2026-08-14T18:01:07Z`) |
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-1.2.7-windows-x64-setup.exe` | 🟩 | present on Helsinki paid_assets store; native Windows PE built on Windows host 2026-08-14 |
| 🤖 **Android** | `restore-privacy-client-1.2.7-android.apk` | 🟩 | node_elgamal.pub pin matches product monopin; Android residual wire (PFS + outer obfs) present; exit_node_elgamal.pub present (multihop exit pin) |
| 🍎 **macOS** | `restore-privacy-client-1.2.7-macos.zip` | 🟩 | Notarized Developer ID monopin (Team SFCBP95595); launch probe alive |
| 📱 **iOS** | `restore-privacy-client-1.2.7-ios.zip` | 🟩 | node_elgamal.pub pin matches product monopin; Team-signed sideload zip |
| 🐧 **Linux** | `restore-privacy-client-1.2.7-linux-x64.tar.gz` | 🟩 | present on Helsinki paid_assets store; packaged on Windows host 2026-08-14 |
...
- Entry probe: **407 ms** via `tcp` (shared base across rows).
- Exit probe: **246 ms** via `tcp` (shared base for multi-hop rows).
...
|-------|------------|-----------|-------------------|------------------|-----|-----|-------|
| on | on | on | 407–412 ms (live base + shape feel) | 246–251 ms (live base + shape feel) | 329 ms | 🟥 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 329.0 ms → red |
| on | on | off | 407–412 ms (live base + shape feel) | n/a (multi-hop off) | 409.5 ms | 🟥 | single-hop residual → entry; shape on; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; AVG 409.5 ms → red |
| on | off | on | 407–412 ms (live base + shape feel) | 246–251 ms (live base + shape feel) | 329 ms | 🟥 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 329.0 ms → red |
| on | off | off | 407–412 ms (live base + shape feel) | n/a (multi-hop off) | 409.5 ms | 🟥 | single-hop residual → entry; shape on; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; AVG 409.5 ms → red |
| off | on | on | 407 ms (live) | 246 ms (live) | 326.5 ms | 🟥 | residual dials exit when multi-hop on; shape off leaner; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 326.5 ms → red |
| off | on | off | 407 ms (live) | n/a (multi-hop off) | 407 ms | 🟥 | single-hop residual → entry; shape off (faster feel); outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; AVG 407.0 ms → red |
| off | off | on | 407 ms (live) | 246 ms (live) | 326.5 ms | 🟥 | residual dials exit when multi-hop on; shape off leaner; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 326.5 ms → red |
| off | off | off | 407 ms (live) | n/a (multi-hop off) | 407 ms | 🟥 | single-hop residual → entry; shape off (faster feel); outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; AVG 407.0 ms → red |
...
|----------|---------|--------|
| **Info** | Automated pass at `2026-08-14T18:01:07Z` | Recorded |
