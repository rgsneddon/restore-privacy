...
- Exit probe: **67 ms** via `tcp` (shared base for multi-hop rows).
...
|-------|------------|-----------|-------------------|------------------|-----|-----|-------|
| on | on | on | 64–69 ms (live base + shape feel) | 67–72 ms (live base + shape feel) | 68 ms | 🟧 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 68.0 ms → amber |
...
| on | off | on | 64–69 ms (live base + shape feel) | 67–72 ms (live base + shape feel) | 68 ms | 🟧 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 68.0 ms → amber |
...
| off | on | on | 64 ms (live) | 67 ms (live) | 65.5 ms | 🟧 | residual dials exit when multi-hop on; shape off leaner; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 65.5 ms → amber |
...
| off | off | on | 64 ms (live) | 67 ms (live) | 65.5 ms | 🟧 | residual dials exit when multi-hop on; shape off leaner; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 65.5 ms → amber |
...
**Overall posture:** **Strong** for residual honesty (`residual_ip_capture`), no public live count, no-phones-home Connect, packaging strip of `*.priv`, tunnel DNS + DoT, Settings transparency. Multi-hop residual is **opt-in** (`RPT_MULTIHOP_ENABLED=1`): residual-via-exit among non-entry catalog peers (exit **Germany (DE)**); default single-hop **Germany (DE)** entry — not full intermediate encapsulation. Catalog monopin **1.0.5**. Product kill-switch is **off by default** (opt-in ``RPT_KILL_SWITCH=1`` only). Installer package confidence is the RAG table at the top of this audit.
...
| Multi-hop residual | Opt-in residual-via-exit (`RPT_MULTIHOP_ENABLED=1`); catalog pubs IS/DE under `product/` (default entry **DE**; RO retired) |
...
| Paid packages | `status_page/downloads.py` monopin **1.0.5**; assets under `status_page/assets/1.0.0/` + VPS `paid_assets` |
| Public web | `status_page/*` catalog **1.0.5** |
...
|----------|---------|--------|
| **Info** | Automated pass at `2026-07-30T22:38:09Z` | Recorded |
...
| Public catalog | **1.0.5** Suite free installers on [status host](https://restoreprivacy.online/); KEYGEN from £3.00/month (VPN APP Shop) |
...
| Code baseline | Catalog **1.0.5** + node **Germany (DE)** |
