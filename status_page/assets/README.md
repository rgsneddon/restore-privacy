...
Current ship: **0.3.6** under `0.3.6/`. Keep prior version dirs only if needed for rollback.

**0.3.6 staging:** All five platform packages staged from `releases/0.3.6/` (Apple
notarized macOS + Team-signed iOS when built on this host; Windows/Android/Linux
may carry-forward prior binaries under **0.3.6** filenames — see
`releases/0.3.6/manifest.json`). Node-only **zram + LUKS2** is a host deploy
