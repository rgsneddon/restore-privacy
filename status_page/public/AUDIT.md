...
| **Public catalog version** | **0.5.4** |
| **Default residual entry** | **United States (US)** (product default on all clients) |
| **Live probe peer** | **Romania (RO):44044** (UDP); status UI TCP **8080** |
| **Audit generated** | **29 July 2026** (`2026-07-29T13:31:14Z`) |
...
## Installer package AUDIT STATE (catalog v0.5.4)
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-0.5.4-windows-x64-setup.exe` | 🟩 | node_elgamal.pub embedded (raw product pin match); PE/MZ magic OK; multihop residual prep markers present (exit_node_elgamal; residual-via-exit when RPT_MULTIHOP_ENABLED=1); exit_node_elgamal.pub raw bytes present (Romania exit pin) |
| 🤖 **Android** | `restore-privacy-client-0.5.4-android.apk` | 🟩 | node_elgamal.pub pin matches product monopin; Android residual wire (PFS + outer obfs) present; exit_node_elgamal.pub present (multihop exit pin) |
| 🍎 **macOS** | `restore-privacy-client-0.5.4-macos.zip` | 🟥 | catalog monopin asset not staged (looked for restore-privacy-client-0.5.4-macos.zip / relative_path 0.5.4/restore-privacy-client-0.5.4-macos.zip under status_page/assets/0.5.4/, releases/0.5.4/, /opt/restore-privacy/paid_assets/0.5.4/, dist/0.5.4/, status_page/assets/, https://135.181.152.10.sslip.io/paid-assets/0.5.4/; default paid store is Helsinki https://135.181.152.10.sslip.io/paid-assets/ — set RPT_ASSET_FETCH_TOKEN to probe, or stage under paid_assets/0.5.4/) |
| 📱 **iOS** | `restore-privacy-client-0.5.4-ios.zip` | 🟥 | catalog monopin asset not staged (looked for restore-privacy-client-0.5.4-ios.zip / relative_path 0.5.4/restore-privacy-client-0.5.4-ios.zip under status_page/assets/0.5.4/, releases/0.5.4/, /opt/restore-privacy/paid_assets/0.5.4/, dist/0.5.4/, status_page/assets/, https://135.181.152.10.sslip.io/paid-assets/0.5.4/; default paid store is Helsinki https://135.181.152.10.sslip.io/paid-assets/ — set RPT_ASSET_FETCH_TOKEN to probe, or stage under paid_assets/0.5.4/) |
| 🐧 **Linux** | `restore-privacy-client-0.5.4-linux-x64.tar.gz` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
...
Latest automated security audit for production node **Romania (RO)** and the in-repo privacy/security gates.
...
| Paid packages | `status_page/downloads.py` monopin **0.5.4**; assets under `status_page/assets/0.5.4/` + VPS `paid_assets` |
| Public web | `status_page/*` catalog **0.5.4** |
...
| Probe | Detail |
|-------|--------|
| TCP `Romania (RO):8080` | ok=True error=None |
| HTTP `http://Romania (RO):8080/status` | code=200 body={'title': 'RESTORE PRIVACY'} |
| UDP `Romania (RO):44044` | sent=True error=None |
...
|----------|---------|--------|
| **Info** | Automated pass at `2026-07-29T13:31:14Z` | Recorded |
...
| Expectation | Notes |
|-------------|--------|
| Product host | **Romania (RO)** |
| Public catalog | **0.5.4** paid installers on [status host](https://restoreprivacy.online/) (£2.45; no free GitHub release downloads) |
...
| Code baseline | Catalog **0.5.4** + node **Romania (RO)** |
| Pass date | **29 July 2026** |
| Machine JSON | `status_page/static/security_audit_latest.json` (when `--write`) |

**Catalog residual peers (reference):** Iceland **82.221.101.241:44044**, Romania **185.146.232.107:44044**, United States **5.161.242.85:44044**.
