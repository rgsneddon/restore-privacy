...
## Installer package AUDIT STATE (catalog v0.5.1)
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-0.5.1-windows-x64-setup.exe` | 🟩 | node_elgamal.pub embedded (raw product pin match); PE/MZ magic OK; multihop residual prep markers present (exit_node_elgamal; residual-via-exit when RPT_MULTIHOP_ENABLED=1); exit_node_elgamal.pub raw bytes present (Romania exit pin) |
| 🤖 **Android** | `restore-privacy-client-0.5.1-android.apk` | 🟩 | node_elgamal.pub pin matches product monopin; Android residual wire (PFS + outer obfs) present; exit_node_elgamal.pub present (multihop exit pin) |
| 🍎 **macOS** | `restore-privacy-client-0.5.1-macos.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 📱 **iOS** | `restore-privacy-client-0.5.1-ios.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 🐧 **Linux** | `restore-privacy-client-0.5.1-linux-x64.tar.gz` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
...
| **multihop_product_pubs** | **PASS** | entry pub present (/Users/russellsneddon/restore-privacy/product/node_elgamal.pub) sha=1b126abfae737c66…; exit pub present (/Users/russellsneddon/restore-privacy/product/exit_no... |
...
| Paid packages | `status_page/downloads.py` monopin **0.5.1**; assets under `status_page/assets/0.5.1/` + VPS `paid_assets` |
| Public web | `status_page/*` catalog **0.5.1** |
...
|----------|---------|--------|
| **Info** | Automated pass at `2026-07-27T08:27:28Z` | Recorded |
...
| Public catalog | **0.5.1** paid installers on [status host](https://restoreprivacy.online/) (£2.45; no free GitHub release downloads) |
...
| Code baseline | Catalog **0.5.1** + node **82.221.101.241** |
