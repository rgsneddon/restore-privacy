...
## Installer package AUDIT STATE (catalog v1.1.2)
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-1.1.2-windows-x64-setup.exe` | 🟩 | node_elgamal.pub embedded (raw product pin match); PE/MZ magic OK; multihop residual prep markers present (exit_node_elgamal; residual-via-exit when RPT_MULTIHOP_ENABLED=1); exit_node_elgamal.pub raw bytes present (Germany DE exit pin) |
| 🤖 **Android** | `restore-privacy-client-1.1.2-android.apk` | 🟩 | node_elgamal.pub pin matches product monopin; Android residual wire (PFS + outer obfs) present; exit_node_elgamal.pub present (multihop exit pin) |
| 🍎 **macOS** | `restore-privacy-client-1.1.2-macos.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 📱 **iOS** | `restore-privacy-client-1.1.2-ios.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 🐧 **Linux** | `restore-privacy-client-1.1.2-linux-x64.tar.gz` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
...
**Overall posture:** **Strong** for residual honesty (`residual_ip_capture`), no public live count, no-phones-home Connect, packaging strip of `*.priv`, tunnel DNS + DoT, Settings transparency. Multi-hop residual is **opt-in** (`RPT_MULTIHOP_ENABLED=1`): residual-via-exit among non-entry catalog peers (exit **Germany (DE)**); default single-hop **Germany (DE)** entry — not full intermediate encapsulation. Catalog monopin **1.1.2**. Product kill-switch is **off by default** (opt-in ``RPT_KILL_SWITCH=1`` only). Installer package confidence is the RAG table at the top of this audit.
...
| Paid packages | `status_page/downloads.py` monopin **1.1.2**; assets under `status_page/assets/1.1.2/` + VPS `paid_assets` |
| Public web | `status_page/*` catalog **1.1.2** |
...
| Public catalog | **1.1.2** paid installers on [status host](https://restoreprivacy.online/) (£3.00/month or £30.00/year KEYGEN; free Suite installers via FREE DOWNLOAD; no free GitHub release downloads) |
...
| Code baseline | Catalog **1.1.2** + node **Germany (DE)** |
