...
| **Public catalog version** | **0.4.8** (live paid packages; package RAG table below may lag until next full audit pass) |
...
Last full RAG snapshot used catalog **v0.4.0** packages; live monopin is **0.4.8**.  
...
| Paid packages | `status_page/downloads.py` monopin **0.4.8** |
...
| Public catalog | **0.4.8** paid installers on [status host](https://restoreprivacy.online/) |
...
| Code baseline | Catalog **0.4.8** + node **82.221.101.241** |
