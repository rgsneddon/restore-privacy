...
| **Audit generated** | **14 August 2026** (`2026-08-14T15:56:25Z`) |
...
## Installer package AUDIT STATE (catalog v1.2.6)
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-1.2.6-windows-x64-setup.exe` | 🟧 | catalog monopin asset not staged (looked for restore-privacy-client-1.2.6-windows-x64-setup.exe / relative_path 1.2.6/restore-privacy-client-1.2.6-windows-x64-setup.exe under status_page/assets/1.2.6/, releases/1.2.6/, /opt/restore-privacy/paid_assets/1.2.6/, dist/1.2.6/, status_page/assets/, https://135.181.152.10.sslip.io/paid-assets/1.2.6/; default paid store is Helsinki https://135.181.152.10.sslip.io/paid-assets/ — set RPT_ASSET_FETCH_TOKEN to probe, or stage under paid_assets/1.2.6/); Windows PE pending native rebuild on Windows host (see client/windows/WINDOWS_HANDOFF_1.2.6.md; not seal-claimed — catalog ticker is Amber, not Red, while handoff is open) |
| 🤖 **Android** | `restore-privacy-client-1.2.6-android.apk` | 🟩 | node_elgamal.pub pin matches product monopin; Android residual wire (PFS + outer obfs) present; exit_node_elgamal.pub present (multihop exit pin) |
| 🍎 **macOS** | `restore-privacy-client-1.2.6-macos.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 📱 **iOS** | `restore-privacy-client-1.2.6-ios.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 🐧 **Linux** | `restore-privacy-client-1.2.6-linux-x64.tar.gz` | 🟥 | catalog monopin asset not staged (looked for restore-privacy-client-1.2.6-linux-x64.tar.gz / relative_path 1.2.6/restore-privacy-client-1.2.6-linux-x64.tar.gz under status_page/assets/1.2.6/, releases/1.2.6/, /opt/restore-privacy/paid_assets/1.2.6/, dist/1.2.6/, status_page/assets/, https://135.181.152.10.sslip.io/paid-assets/1.2.6/; default paid store is Helsinki https://135.181.152.10.sslip.io/paid-assets/ — set RPT_ASSET_FETCH_TOKEN to probe, or stage under paid_assets/1.2.6/) |
...
| Probe | State | Notes |
|-------|-------|-------|
| **multihop_module_flags** | **PASS** | MULTI_HOP_ROUTING_IMPLEMENTED=True (residual-via-exit); Germany peer Germany (DE) (DE monopin / default entry); Singapore peer Singapore (SG) (SG monopin); Germany exit Germany ... |
...
| **multihop_residual_via_exit** | **PASS** | is_multihop_active=True for SG→DE entry→exit path; residual_endpoint dials exit Germany (DE) (residual-via-exit); multi-hop disabled residual stays entry Germany (DE) (default s... |
...
- Entry probe: **70 ms** via `tcp` (shared base across rows).
- Exit probe: **326 ms** via `tcp` (shared base for multi-hop rows).
...
|-------|------------|-----------|-------------------|------------------|-----|-----|-------|
| on | on | on | 70–75 ms (live base + shape feel) | 326–331 ms (live base + shape feel) | 200.5 ms | 🟥 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 200.5 ms → red |
| on | on | off | 70–75 ms (live base + shape feel) | n/a (multi-hop off) | 72.5 ms | 🟥 | single-hop residual → entry; shape on; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; AVG 72.5 ms → red |
| on | off | on | 70–75 ms (live base + shape feel) | 326–331 ms (live base + shape feel) | 200.5 ms | 🟥 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 200.5 ms → red |
| on | off | off | 70–75 ms (live base + shape feel) | n/a (multi-hop off) | 72.5 ms | 🟥 | single-hop residual → entry; shape on; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; AVG 72.5 ms → red |
| off | on | on | 70 ms (live) | 326 ms (live) | 198 ms | 🟥 | residual dials exit when multi-hop on; shape off leaner; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 198.0 ms → red |
| off | on | off | 70 ms (live) | n/a (multi-hop off) | 70 ms | 🟧 | single-hop residual → entry; shape off (faster feel); outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; AVG 70.0 ms → amber |
| off | off | on | 70 ms (live) | 326 ms (live) | 198 ms | 🟥 | residual dials exit when multi-hop on; shape off leaner; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 198.0 ms → red |
| off | off | off | 70 ms (live) | n/a (multi-hop off) | 70 ms | 🟧 | single-hop residual → entry; shape off (faster feel); outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; AVG 70.0 ms → amber |
...
| Paid packages | `status_page/downloads.py` monopin **1.2.6**; assets under `status_page/assets/1.2.6/` + VPS `paid_assets` |
| Public web | `status_page/*` catalog **1.2.6** |
...
| Singapore (SG) | ok=True err=None | code=200 body={'title': 'RESTORE PRIVACY'} | sent=True err=None |
...
|----------|---------|--------|
| **Info** | Automated pass at `2026-08-14T15:56:25Z` | Recorded |
...
| Public catalog | **1.2.6** paid installers on [status host](https://restoreprivacy.online/) (£3.00/month or £30.00/year KEYGEN; free Suite installers via FREE DOWNLOAD; no free GitHub release downloads) |
...
| Code baseline | Catalog **1.2.6** + node **Germany (DE)** |
