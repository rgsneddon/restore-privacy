...
| **Audit generated** | **04 August 2026** (`2026-08-04T20:59:30Z`) |
...
## Installer package AUDIT STATE (catalog v1.2.0)
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-1.2.0-windows-x64-setup.exe` | 🟩 | node_elgamal.pub embedded (raw product pin match); PE/MZ magic OK; multihop residual prep markers present (exit_node_elgamal; residual-via-exit when RPT_MULTIHOP_ENABLED=1); exit_node_elgamal.pub raw bytes present (Germany DE exit pin) |
| 🤖 **Android** | `restore-privacy-client-1.2.0-android.apk` | 🟥 | catalog monopin asset not staged (looked for restore-privacy-client-1.2.0-android.apk / relative_path 1.2.0/restore-privacy-client-1.2.0-android.apk under status_page/assets/1.2.0/, releases/1.2.0/, /opt/restore-privacy/paid_assets/1.2.0/, dist/1.2.0/, status_page/assets/, https://135.181.152.10.sslip.io/paid-assets/1.2.0/; default paid store is Helsinki https://135.181.152.10.sslip.io/paid-assets/ — set RPT_ASSET_FETCH_TOKEN to probe, or stage under paid_assets/1.2.0/) |
| 🍎 **macOS** | `restore-privacy-client-1.2.0-macos.zip` | 🟥 | catalog monopin asset not staged (looked for restore-privacy-client-1.2.0-macos.zip / relative_path 1.2.0/restore-privacy-client-1.2.0-macos.zip under status_page/assets/1.2.0/, releases/1.2.0/, /opt/restore-privacy/paid_assets/1.2.0/, dist/1.2.0/, status_page/assets/, https://135.181.152.10.sslip.io/paid-assets/1.2.0/; default paid store is Helsinki https://135.181.152.10.sslip.io/paid-assets/ — set RPT_ASSET_FETCH_TOKEN to probe, or stage under paid_assets/1.2.0/) |
| 📱 **iOS** | `restore-privacy-client-1.2.0-ios.zip` | 🟥 | catalog monopin asset not staged (looked for restore-privacy-client-1.2.0-ios.zip / relative_path 1.2.0/restore-privacy-client-1.2.0-ios.zip under status_page/assets/1.2.0/, releases/1.2.0/, /opt/restore-privacy/paid_assets/1.2.0/, dist/1.2.0/, status_page/assets/, https://135.181.152.10.sslip.io/paid-assets/1.2.0/; default paid store is Helsinki https://135.181.152.10.sslip.io/paid-assets/ — set RPT_ASSET_FETCH_TOKEN to probe, or stage under paid_assets/1.2.0/) |
| 🐧 **Linux** | `restore-privacy-client-1.2.0-linux-x64.tar.gz` | 🟥 | catalog monopin asset not staged (looked for restore-privacy-client-1.2.0-linux-x64.tar.gz / relative_path 1.2.0/restore-privacy-client-1.2.0-linux-x64.tar.gz under status_page/assets/1.2.0/, releases/1.2.0/, /opt/restore-privacy/paid_assets/1.2.0/, dist/1.2.0/, status_page/assets/, https://135.181.152.10.sslip.io/paid-assets/1.2.0/; default paid store is Helsinki https://135.181.152.10.sslip.io/paid-assets/ — set RPT_ASSET_FETCH_TOKEN to probe, or stage under paid_assets/1.2.0/) |
...
| **multihop_product_pubs** | **PASS** | IS pub present (C:\Users\rgsne\restore_privacy\product\node_elgamal.pub) sha=1b126abfae737c66…; exit/DE pub present (C:\Users\rgsne\restore_privacy\product\exit_node_elgamal.pub... |
...
- Entry probe: **44 ms** via `tcp` (shared base across rows).
...
|-------|------------|-----------|-------------------|------------------|-----|-----|-------|
| on | on | on | 44–49 ms (live base + shape feel) | 79–84 ms (live base + shape feel) | 64 ms | 🟧 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 64.0 ms → amber |
| on | on | off | 44–49 ms (live base + shape feel) | n/a (multi-hop off) | 46.5 ms | 🟧 | single-hop residual → entry; shape on; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; AVG 46.5 ms → amber |
| on | off | on | 44–49 ms (live base + shape feel) | 79–84 ms (live base + shape feel) | 64 ms | 🟧 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 64.0 ms → amber |
| on | off | off | 44–49 ms (live base + shape feel) | n/a (multi-hop off) | 46.5 ms | 🟧 | single-hop residual → entry; shape on; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; AVG 46.5 ms → amber |
| off | on | on | 44 ms (live) | 79 ms (live) | 61.5 ms | 🟧 | residual dials exit when multi-hop on; shape off leaner; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 61.5 ms → amber |
| off | on | off | 44 ms (live) | n/a (multi-hop off) | 44 ms | 🟧 | single-hop residual → entry; shape off (faster feel); outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; AVG 44.0 ms → amber |
| off | off | on | 44 ms (live) | 79 ms (live) | 61.5 ms | 🟧 | residual dials exit when multi-hop on; shape off leaner; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 61.5 ms → amber |
| off | off | off | 44 ms (live) | n/a (multi-hop off) | 44 ms | 🟧 | single-hop residual → entry; shape off (faster feel); outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; AVG 44.0 ms → amber |
...
**Overall posture:** **Strong** for residual honesty (`residual_ip_capture`), no public live count, no-phones-home Connect, packaging strip of `*.priv`, tunnel DNS + DoT, Settings transparency. Multi-hop residual is **opt-in** (`RPT_MULTIHOP_ENABLED=1`): residual-via-exit among non-entry catalog peers (exit **Germany (DE)**); default single-hop **Germany (DE)** entry — not full intermediate encapsulation. Catalog monopin **1.2.0**. Product kill-switch is **off by default** (opt-in ``RPT_KILL_SWITCH=1`` only). Installer package confidence is the RAG table at the top of this audit.
...
| Paid packages | `status_page/downloads.py` monopin **1.2.0**; assets under `status_page/assets/1.2.0/` + VPS `paid_assets` |
| Public web | `status_page/*` catalog **1.2.0** |
...
|----------|---------|--------|
| **Info** | Automated pass at `2026-08-04T20:59:30Z` | Recorded |
...
## 6. Automated checks (this pass — 04 August 2026)
...
| Public catalog | **1.2.0** paid installers on [status host](https://restoreprivacy.online/) (£3.00/month or £30.00/year KEYGEN; free Suite installers via FREE DOWNLOAD; no free GitHub release downloads) |
