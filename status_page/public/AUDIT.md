...
| **Audit generated** | **29 July 2026** (`2026-07-29T11:46:30Z`) |
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-0.5.3-windows-x64-setup.exe` | 🟩 | node_elgamal.pub embedded (raw product pin match); PE/MZ magic OK; multihop residual prep markers present (exit_node_elgamal; residual-via-exit when RPT_MULTIHOP_ENABLED=1); exit_node_elgamal.pub raw bytes present (Romania exit pin) |
| 🤖 **Android** | `restore-privacy-client-0.5.3-android.apk` | 🟩 | node_elgamal.pub pin matches product monopin; Android residual wire (PFS + outer obfs) present; exit_node_elgamal.pub present (multihop exit pin) |
| 🍎 **macOS** | `restore-privacy-client-0.5.3-macos.zip` | 🟥 | catalog monopin asset not staged (looked for restore-privacy-client-0.5.3-macos.zip / relative_path 0.5.3/restore-privacy-client-0.5.3-macos.zip under status_page/assets/0.5.3/, releases/0.5.3/, /opt/restore-privacy/paid_assets/0.5.3/, dist/0.5.3/, status_page/assets/; paid host may still fulfil via private GH / VPS HTTP when uploaded) |
| 📱 **iOS** | `restore-privacy-client-0.5.3-ios.zip` | 🟥 | catalog monopin asset not staged (looked for restore-privacy-client-0.5.3-ios.zip / relative_path 0.5.3/restore-privacy-client-0.5.3-ios.zip under status_page/assets/0.5.3/, releases/0.5.3/, /opt/restore-privacy/paid_assets/0.5.3/, dist/0.5.3/, status_page/assets/; paid host may still fulfil via private GH / VPS HTTP when uploaded) |
| 🐧 **Linux** | `restore-privacy-client-0.5.3-linux-x64.tar.gz` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
...
|----------|---------|--------|
| **Info** | Automated pass at `2026-07-29T11:46:30Z` | Recorded |
...
| Code baseline | Catalog **0.5.3** + node **Iceland (IS)** |
