...
## Installer package AUDIT STATE (catalog v1.2.7)
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-1.2.7-windows-x64-setup.exe` | 🟩 | present on Helsinki paid_assets store (https://135.181.152.10.sslip.io/paid-assets/1.2.7/restore-privacy-client-1.2.7-windows-x64-setup.exe; 39073712 bytes); native Windows PE built on Windows host 2026-08-14 |
...
| 🍎 **macOS** | `restore-privacy-client-1.2.7-macos.zip` | 🟧 | 1.2.7 Apple seal awaits Mac host; Downloads Map still fulfils restore-privacy-client-1.2.6-macos.zip until Mac ships |
...
| 🐧 **Linux** | `restore-privacy-client-1.2.7-linux-x64.tar.gz` | 🟩 | present on Helsinki paid_assets store (https://135.181.152.10.sslip.io/paid-assets/1.2.7/restore-privacy-client-1.2.7-linux-x64.tar.gz; 14689272 bytes); packaged on Windows host 2026-08-14 |
...
**Overall posture:** **Strong** for residual honesty (`residual_ip_capture`), no public live count, no-phones-home Connect, packaging strip of `*.priv`, tunnel DNS + DoT, Settings transparency. Multi-hop residual is **opt-in** (`RPT_MULTIHOP_ENABLED=1`): residual-via-exit among non-entry catalog peers (exit **Germany (DE)**); default single-hop **Germany (DE)** entry — not full intermediate encapsulation. Catalog monopin **1.2.7**. Product kill-switch is **off by default** (opt-in ``RPT_KILL_SWITCH=1`` only). Installer package confidence is the RAG table at the top of this audit.
...
| Paid packages | `status_page/downloads.py` monopin **1.2.7**; assets under `status_page/assets/1.2.7/` + VPS `paid_assets` |
| Public web | `status_page/*` catalog **1.2.7** |
...
| Public catalog | **1.2.7** paid installers on [status host](https://restoreprivacy.online/) (£3.00/month or £30.00/year KEYGEN; free Suite installers via FREE DOWNLOAD; no free GitHub release downloads) |
...
| Code baseline | Catalog **1.2.7** + node **Germany (DE)** |
