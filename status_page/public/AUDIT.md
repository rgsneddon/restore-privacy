...
| **Audit generated** | **3 August 2026** (`2026-08-03T22:21:44Z`) |
...
## Installer package AUDIT STATE (catalog v0.3.6)
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-0.3.6-windows-x64-setup.exe` | 🟥 | catalog monopin asset not staged (looked for restore-privacy-client-0.3.6-windows-x64-setup.exe / relative_path 0.3.6/restore-privacy-client-0.3.6-windows-x64-setup.exe under status_page/assets/0.3.6/, releases/0.3.6/, paid_assets/0.3.6/, dist/0.3.6/, status_page/assets/; paid host may still fulfil via private GH / VPS HTTP when uploaded) |
| 🐧 **Linux** | `restore-privacy-client-0.3.6-linux-x64.tar.gz` | 🟥 | catalog monopin asset not staged (looked for restore-privacy-client-0.3.6-linux-x64.tar.gz / relative_path 0.3.6/restore-privacy-client-0.3.6-linux-x64.tar.gz under status_page/assets/0.3.6/, releases/0.3.6/, paid_assets/0.3.6/, dist/0.3.6/, status_page/assets/; paid host may still fulfil via private GH / VPS HTTP when uploaded) |
| 🍎 **macOS** | `restore-privacy-client-0.3.6-macos.zip` | 🟥 | catalog monopin asset not staged (looked for restore-privacy-client-0.3.6-macos.zip / relative_path 0.3.6/restore-privacy-client-0.3.6-macos.zip under status_page/assets/0.3.6/, releases/0.3.6/, paid_assets/0.3.6/, dist/0.3.6/, status_page/assets/; paid host may still fulfil via private GH / VPS HTTP when uploaded) |
| 📱 **iOS** | `restore-privacy-client-0.3.6-ios.zip` | 🟥 | catalog monopin asset not staged (looked for restore-privacy-client-0.3.6-ios.zip / relative_path 0.3.6/restore-privacy-client-0.3.6-ios.zip under status_page/assets/0.3.6/, releases/0.3.6/, paid_assets/0.3.6/, dist/0.3.6/, status_page/assets/; paid host may still fulfil via private GH / VPS HTTP when uploaded) |
| 🤖 **Android** | `restore-privacy-client-0.3.6-android.apk` | 🟥 | catalog monopin asset not staged (looked for restore-privacy-client-0.3.6-android.apk / relative_path 0.3.6/restore-privacy-client-0.3.6-android.apk under status_page/assets/0.3.6/, releases/0.3.6/, paid_assets/0.3.6/, dist/0.3.6/, status_page/assets/; paid host may still fulfil via private GH / VPS HTTP when uploaded) |
...
| Probe | State | Notes |
|-------|-------|-------|
| **nolog_journald** | **PASS** | config ok: rpt-node.json; unit checks via readable fixture rpt-node.service; unit StandardOutput=null present |
| **no_priv_public_trees** | **PASS** | scanned_roots=9; hits=0 |
...
| **host_privacy_drift** | **PASS** | journald drop-in present (short retention); unit not logging StandardOutput to journal; install_host_privacy.sh recipe present in tree |
| **disk_wipe_readiness** | **PASS** | cryptsetup present: /usr/sbin/cryptsetup; install_disk_encryption.sh present; install_zram_luks.sh present (node-only ram volume) |
| **ephemeral_dry_run** | **PASS** | ephemeral_node.py present (/opt/restore-privacy/scripts/ephemeral_node.py); dry-run mode available; dry-run exit=0 |
...
In-scope probes: **all PASS** (firewall excluded by design).
...
| Probe | State | Notes |
|-------|-------|-------|
| **multihop_module_flags** | **FAIL** | MULTI_HOP_ROUTING_IMPLEMENTED=True (residual-via-exit); entry host 82.221.101.241:44044 (Iceland monopin); exit host '178.105.187.178' != product Romania pin 185.146.232.107 |
| **multihop_product_pubs** | **PASS** | entry pub present (/opt/restore-privacy/product/node_elgamal.pub) sha=1b126abfae737c66…; exit pub present (/opt/restore-privacy/product/exit_node_elgamal.pub) sha=a36a3f38066ece... |
| **multihop_residual_via_exit** | **PASS** | is_multihop_active=True for entry→exit path; residual_endpoint dials exit 178.105.187.178:44044 (residual-via-exit); multi-hop disabled residual stays entry 82.221.101.241 (defa... |
| **multihop_node_host_layout** | **PASS** | present: /opt/restore-privacy/node/install_zram_luks.sh; present: /opt/restore-privacy/node/install_host_privacy.sh; node-only: clients never install LUKS/zram |
...
Latest automated security audit for production node **127.0.0.1** and the in-repo privacy/security gates.
...
| Paid packages | `status_page/downloads.py` monopin **0.3.6**; assets under `status_page/assets/0.3.6/` + VPS `paid_assets` |
| Public web | `status_page/*` catalog **0.3.6** |
...
- Product default host **127.0.0.1**.  
...
| Probe | Detail |
|-------|--------|
| TCP `127.0.0.1:8080` | ok=True error=None |
| HTTP `http://127.0.0.1:8080/status` | code=200 body={'title': 'RESTORE PRIVACY'} |
| UDP `127.0.0.1:44044` | sent=True error=None |
...
|----------|---------|--------|
| **Info** | Automated pass at `2026-08-03T22:21:44Z` | Recorded |
...
## 6. Automated checks (this pass — 3 August 2026)
...
| Code baseline | Catalog **0.3.6** + node **127.0.0.1** |
| Pass date | **3 August 2026** |
