...
| | |
|--|--|
| **Get the app** | [Paid downloads](https://restoreprivacy.online/) — catalog **v0.4.8** (£2.45/month or £27.93/year per platform) |
...
- Connect uses the standard **HELLO** residual path (**flyclient** fast-path removed in catalog **v0.4.8**).
...
- Multi-hop residual is **opt-in** (`RPT_MULTIHOP_ENABLED=1`): exit is a **random non-entry** catalog peer (IS or RO when entry is US; random non-entry peer). Default is single-hop on the chosen entry. **residual-via-exit** routing is implemented (not hop-list-only).
...
**Current catalog (v0.4.8):** the homepage **Download client** box has device/platform and plan selection — **Monthly VPN plan** £2.45 / **Yearly VPN plan** £27.93 (5% off) — plus **Buy now**, which opens Stripe Checkout. You can also open the plan page at **`/pay`**. Card payment uses Stripe’s hosted checkout (Dashboard branding only — not this site’s full CSS). Email delivers **keygen + PPI + download link** (**USE THIS KEYGEN TO UNLOCK RESTORE PRIVACY**).
...
1. Get **`restore-privacy-browser-extension-0.4.8.zip`** from the [0.4.8 release assets](https://github.com/rgsneddon/restore-privacy/releases/tag/0.4.8) (or build from `browser_extension/`).
...
1. On the [status downloads page](https://restoreprivacy.online/), choose **Monthly £2.45** or **Yearly** for **Windows** and download **`restore-privacy-client-0.4.8-windows-x64-setup.exe`** (one-time link after payment).
...
1. On the [status downloads page](https://restoreprivacy.online/), choose **Monthly £2.45** or **Yearly** for **Android** and download **`restore-privacy-client-0.4.8-android.apk`** (one-time link after payment).
...
1. On the [status downloads page](https://restoreprivacy.online/), choose **Monthly £2.45** or **Yearly** for **Linux** and download **`restore-privacy-client-0.4.8-linux-x64.tar.gz`** (one-time link after payment).
2. Unpack and run the bundled installer:

   ```bash
   tar xzf restore-privacy-client-0.4.8-linux-x64.tar.gz
...
- **Monthly £2.45** and **Yearly** pay controls per platform (Windows, Android, macOS, iOS, Linux) — catalog **v0.4.8**
...
**Release scripts:** `scripts/build_release_0.4.8.py`. **Windows multihop PE** (x64 only): `scripts/build_windows_multihop.py` / `scripts\build_windows_multihop.bat` — handoff [`client/windows/WINDOWS_HANDOFF_0.4.8.md`](client/windows/WINDOWS_HANDOFF_0.4.8.md). Release notes: [`scripts/RELEASE_NOTES_0.4.8.md`](scripts/RELEASE_NOTES_0.4.8.md). Catalog **0.4.8** Windows paid package is native PE when rebuilt; prior PE-carry-forward until rebuilt on Windows x64; multihop residual-via-exit remains opt-in (`RPT_MULTIHOP_ENABLED=1` / Settings multi-hop).
