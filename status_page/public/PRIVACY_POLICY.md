...
**Current packages (catalog v0.4.2):** paid installers via [status downloads](https://restoreprivacy.online/) — **monthly (£2.45 GBP)** or **yearly** subscription per platform (Windows · Android · macOS · iOS · Linux — macOS Developer ID notarized; iOS Team-signed sideload). Yearly amount is set in Stripe by the operator. The product **source repository is private**; free permanent public GitHub installer URLs are **not** offered. After payment the status host delivers a **one-time** download (authenticated proxy) and emails a **keygen** + **PPI**. Pre-adjustment Settings defaults (lean residual): run at startup **off**, autoconnect **off**, residual VPN core **always on**, traffic shaping / outer obfuscation / multi-hop **off**. Optional **browser extension** (Chromium MV3, `restore-privacy-browser-extension-0.4.2.zip`) provides **browser-scoped** Connect/Disconnect only — not OS residual TUN.

**STRONG DISCLAIMER — PAYMENT REQUIRED FOR CONNECT:** Access to **Connect** and residual VPN use requires **successful payment** and a **keygen unlock** while the subscription is **active** (**OK**). If payment **fails at any time** (failed checkout, failed charge, refund, dispute, or subscription period ended), status is **EXPIRED** and Connect is cancelled until you renew and re-enter a valid keygen. Stripe session id / keygen are used as **payment entitlement** keys (not a username/password account); status host stores entitlement outcome for Connect checks.
...
- Outer **layer obfuscation** (QUIC-mimic wrapper around RPT frames) is **off by default** (lean residual); when enabled in Settings (or ``RPT_OBFS=1``) it reduces clear ``RPT2`` magic on the wire alone (mitigation; not a DPI-undetectability claim).
...
- **Traffic-shape** features (packet padding, send-side timing jitter, cover/dummy frames) and **outer-layer obfuscation** (QUIC-mimic wrap) are **off by default** (lean residual) on every product residual path; turn them **on** in Settings for stronger residual defenses (Windows/Linux Python, Android VPN service, iOS/macOS Packet Tunnel — when on: pad bucket 128, modest send jitter ≤40ms, periodic cover ~2s). On Python set **`RPT_TRAFFIC_SHAPE=1`** / **`RPT_OBFS=1`** to force on. When enabled they reduce coarse traffic fingerprints and are **not** a guarantee of undetectability against sophisticated DPI.
...
- **Payment entitlement:** after successful paid Checkout the status host records an **active** Connect entitlement for that Stripe session (and binds Stripe `payment_intent` id so refunds without session metadata still revoke), mints a **keygen**, and emails **keygen + PPI + download**. Clients must **accept the licence** and **activate the keygen** for Connect (**OK**). Webhook-observed **payment failures / refunds / disputes / period end** set the entitlement to **failed/revoked** (**EXPIRED**): apps hard-lock with **renew your licence *here*** and a platform payment portal link. Clients re-check `/api/connect-entitlement` on Connect (no password account; keygen and/or session id).
...
- **Does not** publish free permanent GitHub `releases/download` installer buttons. Catalog **v0.4.2** packages are fulfilled **after payment** on [status downloads](https://restoreprivacy.online/) via a **one-time** proxy download (private source repository).
...
| **Coarse wire fingerprints** | Outer obfuscation and traffic shaping (padding / jitter / cover) are **off by default** (lean residual); when turned **on** in Settings they act as **mitigations** (not undetectability). |
