...
**Current packages (catalog v0.4.2):** paid installers via [status downloads](https://restoreprivacy.online/) — **monthly (£2.45 GBP)** or **yearly** subscription per platform (Windows · Android · macOS · iOS · Linux — macOS Developer ID notarized; iOS Team-signed sideload). Yearly amount is set in Stripe by the operator. The product **source repository is private**; free permanent public GitHub installer URLs are **not** offered. After payment the status host delivers a **one-time** download (authenticated proxy) and emails a **keygen** + **PPI**. Pre-adjustment Settings defaults (lean residual): run at startup **off**, autoconnect **off**, residual VPN core **always on**, traffic shaping / outer obfuscation / multi-hop **off**.
