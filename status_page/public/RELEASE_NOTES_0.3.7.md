# Restore Privacy 0.3.7

**Catalog monopin 0.3.7** — subscription keygen unlock, homepage licence/subscription copy, entry-only wipe display, fulfilment SMTP, audit last-run line.

## Highlights

- **Keygen unlock:** unique RPT-KEY-… minted on paid Stripe checkout; fulfilment email includes keygen + PPI + download link (**USE THIS KEYGEN TO UNLOCK YOUR RESTORE PRIVACY TRIAL**). Clients: Install → accept licence → enter keygen; Connect only while subscription active (online status-host re-check; revoke locks product).
...
Windows PE multihop residual is in the paid catalog asset `restore-privacy-client-0.3.7-windows-x64-setup.exe` (see handoff).
...
- Paid fulfilment: private GH Release assets and/or `status_page/assets/0.3.7/` + Iceland VPS paid_assets store (`scripts/host_paid_assets_vps.py`)
...
2. Multi-hop on **Linux 0.3.7**: `RPT_MULTIHOP_ENABLED=1` → residual via Romania  
3. Multi-hop on **Windows 0.3.7** (rebuilt PE): `RPT_MULTIHOP_ENABLED=1` → residual via Romania  
4. Multi-hop on **Android 0.3.7** with multihop define/env when available  
5. Paid download for **0.3.7** after assets are on status host / private GH release / VPS paid_assets  
