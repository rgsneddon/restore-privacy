...
| **Audit generated** | **2 August 2026** (`2026-08-02T15:42:27Z`) |
...
## Installer package AUDIT STATE (catalog v1.1.1)
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-1.1.1-windows-x64-setup.exe` | 🟩 | node_elgamal.pub embedded (raw product pin match); PE/MZ magic OK; multihop residual prep markers present (exit_node_elgamal; residual-via-exit when RPT_MULTIHOP_ENABLED=1); exit_node_elgamal.pub raw bytes present (Germany DE exit pin) |
| 🤖 **Android** | `restore-privacy-client-1.1.1-android.apk` | 🟩 | node_elgamal.pub pin matches product monopin; Android residual wire (PFS + outer obfs) present; exit_node_elgamal.pub present (multihop exit pin) |
| 🍎 **macOS** | `restore-privacy-client-1.1.1-macos.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 📱 **iOS** | `restore-privacy-client-1.1.1-ios.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
| 🐧 **Linux** | `restore-privacy-client-1.1.1-linux-x64.tar.gz` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
...
- Entry probe: **56 ms** via `tcp` (shared base across rows).
- Exit probe: **66 ms** via `tcp` (shared base for multi-hop rows).
...
|-------|------------|-----------|-------------------|------------------|-----|-----|-------|
| on | on | on | 56–61 ms (live base + shape feel) | 66–71 ms (live base + shape feel) | 63.5 ms | 🟧 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 63.5 ms → amber |
| on | on | off | 56–61 ms (live base + shape feel) | n/a (multi-hop off) | 58.5 ms | 🟧 | single-hop residual → entry; shape on; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; AVG 58.5 ms → amber |
| on | off | on | 56–61 ms (live base + shape feel) | 66–71 ms (live base + shape feel) | 63.5 ms | 🟧 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 63.5 ms → amber |
| on | off | off | 56–61 ms (live base + shape feel) | n/a (multi-hop off) | 58.5 ms | 🟧 | single-hop residual → entry; shape on; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; AVG 58.5 ms → amber |
| off | on | on | 56 ms (live) | 66 ms (live) | 61 ms | 🟧 | residual dials exit when multi-hop on; shape off leaner; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 61.0 ms → amber |
| off | on | off | 56 ms (live) | n/a (multi-hop off) | 56 ms | 🟧 | single-hop residual → entry; shape off (faster feel); outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; AVG 56.0 ms → amber |
| off | off | on | 56 ms (live) | 66 ms (live) | 61 ms | 🟧 | residual dials exit when multi-hop on; shape off leaner; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 61.0 ms → amber |
| off | off | off | 56 ms (live) | n/a (multi-hop off) | 56 ms | 🟧 | single-hop residual → entry; shape off (faster feel); outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; AVG 56.0 ms → amber |
...
**Overall posture:** **Strong** for residual honesty (`residual_ip_capture`), no public live count, no-phones-home Connect, packaging strip of `*.priv`, tunnel DNS + DoT, Settings transparency. Multi-hop residual is **opt-in** (`RPT_MULTIHOP_ENABLED=1`): residual-via-exit among non-entry catalog peers (exit **Germany (DE)**); default single-hop **Germany (DE)** entry — not full intermediate encapsulation. Catalog monopin **1.1.1**. Product kill-switch is **off by default** (opt-in ``RPT_KILL_SWITCH=1`` only). Installer package confidence is the RAG table at the top of this audit.
...
| Paid packages | `status_page/downloads.py` monopin **1.1.1**; assets under `status_page/assets/1.1.1/` + VPS `paid_assets` |
| Public web | `status_page/*` catalog **1.1.1** |
...
|----------|---------|--------|
| **Info** | Automated pass at `2026-08-02T15:42:27Z` | Recorded |
...
## 6. Automated checks (this pass — 2 August 2026)
...
| Public catalog | **1.1.1** paid installers on [status host](https://restoreprivacy.online/) (£3.00/month or £30.00/year KEYGEN; free Suite installers via FREE DOWNLOAD; no free GitHub release downloads) |
...
| Code baseline | Catalog **1.1.1** + node **Germany (DE)** |
| Pass date | **2 August 2026** |
