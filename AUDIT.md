...
## Installer package AUDIT STATE (catalog v0.3.9)
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-0.3.9-windows-x64-setup.exe` | 🟩 | node_elgamal.pub embedded (raw product pin match); PE/MZ magic OK; multihop residual prep markers present (exit_node_elgamal; residual-via-exit when RPT_MULTIHOP_ENABLED=1); exit_node_elgamal.pub raw bytes present (Romania exit pin); privacy-scale Settings + ping stats in frozen client |
| 🤖 **Android** | `restore-privacy-client-0.3.9-android.apk` | 🟩 | node_elgamal.pub pin matches product monopin; Android residual wire (PFS + outer obfs) present; exit_node_elgamal.pub present (multihop exit pin) |
| 🍎 **macOS** | `restore-privacy-client-0.3.9-macos.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin); Mac rebuild/re-sign per APPLE_HANDOFF_0.3.9 |
| 📱 **iOS** | `restore-privacy-client-0.3.9-ios.zip` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin); Mac rebuild/re-sign per APPLE_HANDOFF_0.3.9 |
| 🐧 **Linux** | `restore-privacy-client-0.3.9-linux-x64.tar.gz` | 🟩 | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
...
| Paid packages | `status_page/downloads.py` monopin **0.3.9**; assets under `status_page/assets/0.3.9/` + VPS `paid_assets` |
| Public web | `status_page/*` catalog **0.3.9** |
...
| Public catalog | **0.3.9** paid installers on [status host](https://restoreprivacy.online/) (£2.45; no free GitHub release downloads) |
...
| Code baseline | Catalog **0.3.9** + node **82.221.101.241** |
