...
| **Live probe peer** | **VPN node:44044** (UDP); status UI TCP **8080** |
| **Audit generated** | **30 July 2026** (`2026-07-30T00:00:28Z`) |
...
| **title_only_status** | **FAIL** | http probe not ok: <urlopen error timed out> |
...
Honesty: **residual-via-exit when multi-hop enabled; not full intermediate onion encapsulation; default single-hop United States entry**.

| Probe | State | Notes |
|-------|-------|-------|
| **multihop_module_flags** | **FAIL** | MULTI_HOP_ROUTING_IMPLEMENTED=True (residual-via-exit); Iceland peer Iceland (IS) (IS monopin); exit host 'Germany (DE)' != product Romania pin VPN node; United States peer Unit... |
| **multihop_product_pubs** | **PASS** | IS pub present (/Users/russellsneddon/restore-privacy/product/node_elgamal.pub) sha=1b126abfae737c66…; RO pub present (/Users/russellsneddon/restore-privacy/product/exit_node_el... |
| **multihop_residual_via_exit** | **FAIL** | is_multihop_active=True for US→RO entry→exit path; residual_endpoint dials exit Germany (DE) (residual-via-exit); default single-hop residual 'Germany (DE)' != US United States ... |
...
Latest automated security audit for production node **VPN node** and the in-repo privacy/security gates.
...
| Node status TCP :8080 | UNREACHABLE |
| Node `/status` HTTP | FAIL — title-only=False |
...
| Probe | Detail |
|-------|--------|
| TCP `VPN node:8080` | ok=False error=timed out |
| HTTP `http://VPN node:8080/status` | code=None body=None |
| UDP `VPN node:44044` | sent=True error=None |
...
|----------|---------|--------|
| **Info** | Automated pass at `2026-07-30T00:00:28Z` | Recorded |
...
| Code baseline | Catalog **0.5.7** + node **VPN node** |
