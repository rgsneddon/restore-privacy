...
## Installer package AUDIT STATE (last full RAG snapshot: catalog v0.4.0; live monopin **0.4.5**)
...
| ðŸŽ **macOS** | `restore-privacy-client-0.4.0-macos.zip` | ðŸŸ© | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin); Mac rebuild/re-sign per APPLE_HANDOFF_0.4.0 |
...
| ðŸ§ **Linux** | `restore-privacy-client-0.4.0-linux-x64.tar.gz` | ðŸŸ© | node_elgamal.pub pin matches product monopin; archive present (structural residual gates N/A in this pass); exit_node_elgamal.pub present (multihop exit pin) |
...
|-------|------------|-----------|-------------------|------------------|-----|-------|
| on | on | off | 43—63 ms | n/a (multi-hop off) | ðŸŸ§ | single-hop residual → entry; shape on; outer obfs on (QUIC-mimic, ~0 ms RTT) |
| on | on | on | 43—63 ms | 37—57 ms | ðŸŸ§ | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs on (QUIC-mimic, ~0 ms RTT) |
| on | off | off | 43—63 ms | n/a (multi-hop off) | ðŸŸ§ | single-hop residual → entry; shape on; outer obfs off (bare RPT, ~0 ms RTT) |
| on | off | on | 43—63 ms | 37—57 ms | ðŸŸ§ | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs off (bare RPT, ~0 ms RTT) |
| off | on | off | 38—58 ms | n/a (multi-hop off) | ðŸŸ§ | single-hop residual → entry; shape off (faster feel); outer obfs on (QUIC-mimic, ~0 ms RTT) |
| off | on | on | 38—58 ms | 32—52 ms | ðŸŸ§ | residual dials exit when multi-hop on; shape off leaner; outer obfs on (QUIC-mimic, ~0 ms RTT) |
| off | off | off | 38—58 ms | n/a (multi-hop off) | ðŸŸ§ | single-hop residual → entry; shape off (faster feel); outer obfs off (bare RPT, ~0 ms RTT) |
| off | off | on | 38—58 ms | 32—52 ms | ðŸŸ§ | residual dials exit when multi-hop on; shape off leaner; outer obfs off (bare RPT, ~0 ms RTT) |
...
| Paid packages | `status_page/downloads.py` monopin **0.4.5**; assets under `status_page/assets/0.4.0/` + VPS `paid_assets` |
| Public web | `status_page/*` catalog **0.4.5** |
