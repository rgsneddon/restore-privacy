...
| **Audit generated** | **2 August 2026** (`2026-08-02T13:59:54Z`) |
...
|----------|---------|-------|-------|
| 🪟 **Windows** | `restore-privacy-client-1.0.6-windows-x64-setup.exe` | 🟩 | node_elgamal.pub embedded (raw product pin match); PE/MZ magic OK; multihop residual prep markers present (exit_node_elgamal; residual-via-exit when RPT_MULTIHOP_ENABLED=1); exit_node_elgamal.pub raw bytes present (Germany DE exit pin) |
...
| 📱 **iOS** | `restore-privacy-client-1.0.6-ios.zip` | 🟧 | archive present (structural residual gates N/A in this pass); node_elgamal.pub not found inside package (pin not verified) |
...
- Entry probe: **45 ms** via `tcp` (shared base across rows).
- Exit probe: **63 ms** via `tcp` (shared base for multi-hop rows).
...
|-------|------------|-----------|-------------------|------------------|-----|-----|-------|
| on | on | on | 45–50 ms (live base + shape feel) | 63–68 ms (live base + shape feel) | 56.5 ms | 🟧 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 56.5 ms → amber |
| on | on | off | 45–50 ms (live base + shape feel) | n/a (multi-hop off) | 47.5 ms | 🟧 | single-hop residual → entry; shape on; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; AVG 47.5 ms → amber |
| on | off | on | 45–50 ms (live base + shape feel) | 63–68 ms (live base + shape feel) | 56.5 ms | 🟧 | residual dials exit when multi-hop on; shape on adds modest jitter/cover feel; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 56.5 ms → amber |
| on | off | off | 45–50 ms (live base + shape feel) | n/a (multi-hop off) | 47.5 ms | 🟧 | single-hop residual → entry; shape on; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; AVG 47.5 ms → amber |
| off | on | on | 45 ms (live) | 63 ms (live) | 54 ms | 🟧 | residual dials exit when multi-hop on; shape off leaner; outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 54.0 ms → amber |
| off | on | off | 45 ms (live) | n/a (multi-hop off) | 45 ms | 🟧 | single-hop residual → entry; shape off (faster feel); outer obfs on (QUIC-mimic, ~0 ms RTT); entry RTT live probe this host; AVG 45.0 ms → amber |
| off | off | on | 45 ms (live) | 63 ms (live) | 54 ms | 🟧 | residual dials exit when multi-hop on; shape off leaner; outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; exit RTT live probe this host; AVG 54.0 ms → amber |
| off | off | off | 45 ms (live) | n/a (multi-hop off) | 45 ms | 🟧 | single-hop residual → entry; shape off (faster feel); outer obfs off (bare RPT, ~0 ms RTT); entry RTT live probe this host; AVG 45.0 ms → amber |
...
| Paid packages | `status_page/downloads.py` monopin **1.0.6**; assets under `status_page/assets/1.0.6/` + VPS `paid_assets` |
...
|----------|---------|--------|
| **Info** | Automated pass at `2026-08-02T13:59:54Z` | Recorded |
...
| Public catalog | **1.0.6** paid installers on [status host](https://restoreprivacy.online/) (£2.45; no free GitHub release downloads) |
