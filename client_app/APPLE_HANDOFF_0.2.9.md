# Apple handoff — Restore Privacy **0.2.9**

Production RPT node: **82.221.101.241:44044** (UDP).

## Public package status (current)

| Asset | Signing (GitHub Release **0.2.9**) |
|-------|-------------------------------------|
| `restore-privacy-client-0.2.9-macos.zip` | **Developer ID Application** (SFCBP95595) + **notarized** (stapled) |
| `restore-privacy-client-0.2.9-ios.zip` | **Apple Distribution** Team-signed sideload (Runner + Packet Tunnel) |

Both inject **`node_elgamal.pub` only**. No `*.priv` in packages. Per-device Ed25519 is generated on first run.

Download: https://github.com/rgsneddon/restore-privacy/releases/tag/0.2.9

**Do not treat 0.2.9 public Apple assets as prep-only.** Prep-stage wording applies only if you are rebuilding from source before re-sign.
...
4. Re-run `scripts/sign_and_notarize_macos.py` and replace the GitHub **0.2.9** macOS zip.
...
1. Checkout tag **0.2.9** (or current `main` with 0.2.9 surfaces).
...
6. Package via `scripts/build_release_0.2.9.py` and attach to GitHub Release **0.2.9** if replacing assets.
