# Apple handoff — Restore Privacy **0.3.0**

Production RPT node: **82.221.101.241:44044** (UDP).

## Public package status (current)

| Asset | Signing (GitHub Release **0.3.0**) |
|-------|-------------------------------------|
| `restore-privacy-client-0.3.0-macos.zip` | **Developer ID Application** (SFCBP95595) + **notarized** (stapled) |
| `restore-privacy-client-0.3.0-ios.zip` | **Apple Distribution** Team-signed sideload (Runner + Packet Tunnel) |

Both inject **`node_elgamal.pub` only**. No `*.priv` in packages. Per-device Ed25519 is generated on first run.

Download: https://github.com/rgsneddon/restore-privacy/releases/tag/0.3.0

**Do not treat 0.3.0 public Apple assets as prep-only.** Prep-stage wording applies only if you are rebuilding from source before re-sign.
...
4. Re-run `scripts/sign_and_notarize_macos.py` and replace the GitHub **0.3.0** macOS zip.
...
1. Checkout tag **0.3.0** (or current `main` with 0.3.0 surfaces).
...
6. Package via `scripts/build_release_0.3.0.py` and attach to GitHub Release **0.3.0** if replacing assets.
