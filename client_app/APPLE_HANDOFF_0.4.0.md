# Apple handoff — Restore Privacy 0.4.0
...
grep productVersion lib/rpt_config.dart   # expect 0.4.0
grep '^version:' pubspec.yaml            # expect 0.4.0+
```
...
# - macOS: Developer ID Application, notarize, staple → restore-privacy-client-0.4.0-macos.zip
# - iOS: Team-signed sideload package → restore-privacy-client-0.4.0-ios.zip
```
...
- `releases/0.4.0/`
- Iceland VPS: `/opt/restore-privacy/paid_assets/0.4.0/`
  (`python scripts/host_paid_assets_vps.py --stage --upload`)
- (optional) `status_page/assets/0.4.0/` for Render-local fallback

Then buyers get packages via paid fulfilment only (token grant / VPS fetch secret).

Catalog monopin and status-host BUY buttons must stay **0.4.0** (`status_page/downloads.py` `RELEASE_VERSION`, `client/VERSION`, Flutter `productVersion`).

## Honesty — staged Apple packages

Live notarization / App Store submission is **not** performed on Windows CI hosts.
This handoff is the Mac operator path for catalog monopin **0.4.0**.

**Mac rebuild (this ship):** `restore-privacy-client-0.4.0-macos.zip` is
Developer ID signed + notarized with `CFBundleShortVersionString` **0.4.0**.
`restore-privacy-client-0.4.0-ios.zip` is Team-signed sideload with marketing
version **0.4.0**. Both include product entry + exit ElGamal **pubs** only
...
```bash
python scripts/host_paid_assets_vps.py --stage --upload --version 0.4.0 --force
```

Audit helper:

```bash
python -c "from apple_package_audit import audit_catalog_apple_packages; import json; print(json.dumps(audit_catalog_apple_packages(version='0.4.0'), indent=2))"
```
