...
enum RptSecrets {
  ...
  static func secretsDirectory(fileManager: FileManager = .default) -> URL? {
    ...
    // Dev fallback when App Group is not configured
    #if os(macOS)
    return fileManager.homeDirectoryForCurrentUser
      .appendingPathComponent(".restore-privacy/secrets", isDirectory: true)
    #else
    // iOS: application Support / Documents for host; extension needs App Group
  ...
  static func filesPresent(fileManager: FileManager = .default) -> Bool {
    ...
    return fileManager.fileExists(atPath: c.path)
      && fileManager.fileExists(atPath: n.path)
  }
