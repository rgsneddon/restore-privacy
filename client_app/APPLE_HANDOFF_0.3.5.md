# Apple handoff — Restore Privacy **0.3.5**

Prior catalog **0.3.0** remains published; **0.3.5** is the current Apple-fixed catalog.

Production RPT node: **82.221.101.241:44044** (UDP).

## Public package status (current)

| Asset | Signing (GitHub Release **0.3.5**) |
|-------|-------------------------------------|
| `restore-privacy-client-0.3.5-macos.zip` | **Developer ID Application** (SFCBP95595) + **notarized** (stapled) |
| `restore-privacy-client-0.3.5-ios.zip` | **Apple Distribution** Team-signed sideload (Runner + Packet Tunnel) |

Both inject **`node_elgamal.pub` only**. No `*.priv` in packages. Per-device Ed25519 is generated on first run.

Download: https://github.com/rgsneddon/restore-privacy/releases/tag/0.3.5

**Do not treat 0.3.5 public Apple assets as prep-only.** Prep-stage wording applies only if you are rebuilding from source before re-sign.
...
4. Re-run `scripts/sign_and_notarize_macos.py` and replace the GitHub **0.3.5** macOS zip.
...
The **public** `restore-privacy-client-0.3.5-macos.zip` is **Developer ID + notarized** and deliberately **omits host Network Extension** so the app opens for all downloaders (host NE without a matching DevID profile is AMFI-killed). That zip is **not** full host-NE residual by itself.
...
1. Checkout tag **0.3.5** (or current `main` with 0.3.5 surfaces).
...
6. Package via `scripts/build_release_0.3.5.py` and attach to GitHub Release **0.3.5** if replacing assets.
