...
| Protocol | **RPT2** (not WireGuard / OpenVPN) â€” see `client/connect.py`, `node/handshake.py` |
| Method channel | **`restore_privacy/vpn`** â€” methods `connect`, `disconnect` (see `lib/vpn_controller.dart`) |
| Connect args | `host`, `port`, `fullTunnel`, `sessionName`, `route`, `autoConnect` |
| Result map | `{ ok, message, vpnIp?, fullTunnelActive?, hostOnlySession? }` â€” use `lib/connect_status.dart` |
...
- `client_ed25519.priv` â€” 32 raw bytes  
- `node_elgamal.pub` â€” node ElGamal public key bytes  
...
```bash
# example â€” from a machine that can SSH to the node
...
- [ios/BUILD_ON_MAC.md](ios/BUILD_ON_MAC.md) â€” Packet Tunnel + signing  
- [macos/BUILD_ON_MAC.md](macos/BUILD_ON_MAC.md) â€” Network Extension + notarization  

Native prep stubs (drag into Xcode targets as described in those docs):

- `ios/NativePrep/` â€” method channel + Packet Tunnel skeleton  
- `macos/NativePrep/` â€” method channel + Packet Tunnel skeleton  
- `ios/NativePrep/RPT_PROTOCOL.md` â€” handshake/DATA outline for Swift  
...
2. **Packet Tunnel** targets (`ios/PacketTunnel`, `macos/PacketTunnel`) implementing UK gate â†’ secrets â†’ handshake â†’ full-tunnel settings â†’ packetFlow/UDP DATA + keepalive.  
3. Host method channel `restore_privacy/vpn` registered from iOS `AppDelegate` / macOS `MainFlutterWindow`; product connect succeeds **only** when Packet Tunnel is active. Host-side RPT2 HELLO is diagnostic-only (never a false â€œConnectedâ€ that leaves residual ISP IP unchanged).  
...
## Operator checklist â€” enable real Packet Tunnel VPN

Do these **in order**. Entitlement *files* are already patched; you still must register them with Apple and sign with your Team.

### 1. Developer portal ([developer.apple.com](https://developer.apple.com) â†’ Identifiers)

1. Sign in with the team that owns **SFCBP95595** (or your team if you change IDs).
2. **App Groups** â†’ create/register: `group.com.restoreprivacy.shared`.
3. **App ID** `com.restoreprivacy.restorePrivacyClient` (host):
   - Capability **Network Extensions** â†’ enable **Packet Tunnel**
   - Capability **App Groups** â†’ select `group.com.restoreprivacy.shared`
4. **App ID** `com.restoreprivacy.restorePrivacyClient.PacketTunnel` (extension):
   - Same **Network Extensions â†’ Packet Tunnel**
...
### 2. Xcode â€” pick Team (both platforms)
...
1. **Signing & Capabilities** â†’ Team **SFCBP95595** (Russell Sneddon).
2. **Automatically manage signing** = ON.
3. Confirm capabilities appear (Network Extensions / Packet Tunnel, App Groups). If Xcode offers to â€œfixâ€ entitlements mismatches, prefer keeping the repo plists above.
...
| Embed | Runner â†’ **Embed Foundation Extensions** â†’ `PacketTunnel.appex` |

**You only need to:** open the workspace, pick Team on Runner + PacketTunnel (Automatic Signing), and ensure the portal App ID for `.PacketTunnel` exists with Network Extensions + App Group. If Xcode shows a profile error, fix the portal App ID â€” do not create a second Packet Tunnel target.
...
| Check | Expected |
|-------|----------|
| App status | `Connected â€” tunnel IP 10.88.0.x` **and** system VPN indicator on |
| macOS menu bar / iOS status bar | VPN active |
| Browser / `curl ifconfig.me` | **Not** your home residual IP (node egress) |
| If Packet Tunnel failed | Honest `ok: false` residual-IP message (not a false â€œConnectedâ€) |
...
*Apple could not verify â€œrestore_privacy_clientâ€ is free of malwareâ€¦*
