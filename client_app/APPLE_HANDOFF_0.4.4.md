# Apple handoff — Restore Privacy 0.4.4
...
7. **Privacy-scale Settings (0.4.4 lean-off defaults)** — traffic shaping / outer obfuscation / multi-hop toggles with honest explainers; factory defaults **Off** for optional residual scale (core residual always required). **Hot-apply** while connected where the platform residual shell supports it (multi-hop re-establishes residual).
...
# - macOS: Developer ID Application, notarize, staple → restore-privacy-client-0.4.4-macos.zip
# - iOS: Team-signed sideload package → restore-privacy-client-0.4.4-ios.zip
```
