# Mac / iOS handoff for Restore Privacy **0.1.6**
...
| File | Notes |
|------|--------|
| `restore-privacy-client-0.1.6-macos.zip` | Staged package for Mac work (may be prior-build payload renamed for catalog continuity) |
| `restore-privacy-client-0.1.6-ios.zip` | Staged sideload package (same) |
| `SHA256SUMS.json` / `manifest.json` | Hashes and sizes |

**Fresh source of truth for Apple work:** git clone `main` (tag `0.1.6`) and open:
...
cd restore-privacy
git checkout 0.1.6   # or main after release
cd client_app
flutter pub get
# macOS
