# Apple handoff — Restore Privacy **0.5.6**

**Monopin / this build:** `0.5.6`
...
| 2. **Team residual NE re-sign** (required) | `build_release_0.5.6.py --apple-only` runs `apple_ship_gates.run_residual_team_resign` → copy `restore_privacy_client.residual-team.app` signed with **Apple Development** + Mac Team NE profiles | Residual Connect works on **this Mac** |
...
| 5. **Host paid assets (macOS + iOS + Linux)** | `host_paid_assets_vps.py --stage --upload --version 0.5.6 --force` with `RPT_SSH_KEY=~/.ssh/id_ed25519_restore_privacy_eu` (root@135.181.152.10) — or `--apple-only --host-paid` after Apple seal; Linux tgz is also staged from this Mac | Helsinki `paid_assets/0.5.6/` basenames `…-macos.zip`, `…-ios.zip`, `…-linux-x64.tar.gz` |
| 6. Catalog pin | `client/VERSION` + `downloads.RELEASE_VERSION` = **0.5.6**; live `/api/catalog-version` after Render deploy | Download links = monopin only |
| 7. **Windows + Android (deferred)** | Native PE + APK rebuild/upload from the **Windows machine** (see `WINDOWS_HANDOFF_0.5.6.md`) | Do not treat Darwin CF Win/Android as the final native seal |

**Do not skip step 2** for residual Connect testing. Opt-out only for non-residual CI:

```bash
RPT_SKIP_RESIDUAL_TEAM=1 python3 scripts/build_release_0.5.6.py --apple-only
# or: python3 scripts/build_release_0.5.6.py --apple-only --skip-residual-team
```
...
python3 scripts/build_release_0.5.6.py
...
```text
paid_assets/0.5.6/restore-privacy-client-0.5.6-macos.zip
paid_assets/0.5.6/restore-privacy-client-0.5.6-ios.zip
paid_assets/0.5.6/restore-privacy-client-0.5.6-linux-x64.tar.gz
```

Windows/Android native installers: rebuild and re-upload from the Windows host
(`WINDOWS_HANDOFF_0.5.6.md`). Darwin may leave CF placeholders under the same
...
```text
releases/0.5.6/restore-privacy-client-0.5.6-macos.zip
```
...
`CFBundleShortVersionString` must equal **0.5.6**. Carry-forward renames of older zips are refused.
...
|----------|---------|---------|-------|
| Catalog macOS zip | **Developer ID Application** + notarized (stapled) | **SFCBP95595** | `spctl` accepted; CFBundle **0.5.6**; residual IPv4 always-on |
...
```text
/opt/restore-privacy/paid_assets/0.5.6/restore-privacy-client-0.5.6-macos.zip
  bytes=20979936  sha256=15805eecd4ad15d795fa7ce9ae0bfabeb66336c0bd33dc9d4c34930c945573cf
/opt/restore-privacy/paid_assets/0.5.6/restore-privacy-client-0.5.6-ios.zip
  bytes=9349814   sha256=6d09d5744068a67276fc83e5764603c4cdd0ef096ed9b444638914174869999b
RPT_CATALOG_VERSION=0.5.6  (rpt-paid-assets.service)
```
