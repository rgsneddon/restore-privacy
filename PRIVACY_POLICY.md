...
- You get the **latest installer for that OS that is listed on the Downloads Map** (fulfilled from the Helsinki package store). Platforms can be on different versions (for example Windows 1.2.5 while Android is 1.2.6) until that platform is rebuilt and the map is updated.
