...
**Windows multihop PE** must be built on Windows x64::
  ...
  # or:  python scripts/build_release_0.5.6.py --windows-only
...
Handoff: ``client/windows/WINDOWS_HANDOFF_0.5.6.md``
"""
...
def stage_windows_exe(*, force_rebuild: bool = False) -> Path:
    ...
    if force_rebuild or sys.platform.startswith("win"):
        try:
            ...
            if force_rebuild:
                raise
    # Prefer already-rebuilt 0.5.6 SFX if present with correct pin
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.windows_only:
        ...
        manifest = {
            ...
            "notes": "0.5.6 Windows multihop residual PE + window_foreground (build_windows_multihop / --windows-only)",
        }
        if man_path.is_file():
            try:
