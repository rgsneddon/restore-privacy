#!/usr/bin/env python3
"""Build Restore Privacy residual VPN monopin 1.2.3 installers and stage for Helsinki.
...
are re-pinned to 1.2.3 (carry-forward prior 1.2.2) for store layout; operators should replace with native
rebuilds when a Windows/Linux build agent is available.

Usage::

  python3 scripts/build_suite_1.2.3.py
  python3 scripts/build_suite_1.2.3.py --skip-build   # stage/copy only
  python3 scripts/build_suite_1.2.3.py --host-paid     # stage + Helsinki upload
"""
