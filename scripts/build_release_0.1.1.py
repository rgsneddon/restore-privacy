#!/usr/bin/env python3
"""Build / stage Restore Privacy client packages for release 0.1.1.

Apple packages include Packet Tunnel Team signing + App Group secrets seed.
Windows/Android are staged from 0.1.0 when not rebuilt on this host.
...
def write_version_files() -> None:
    ...
    if inst.is_file():
        ...
        t2 = t
        for old in ("0.1.0", "0.0.9", "0.0.8", "0.0.7"):
