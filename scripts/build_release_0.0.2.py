#!/usr/bin/env python3
"""Build Restore Privacy client packages for release 0.0.2.
...
def build_windows_installer_exe(client_onedir: Path) -> Path:
    ...
    # Embed entire onedir under payload/
    ...
    if b"BEGIN PRIVATE" in raw or b"client_ed25519.priv" in raw:
        raise RuntimeError("Installer binary appears to embed private key material")
...
def build_android_apk() -> Path | None:
    """flutter build apk, or stage existing release APK under 0.0.2 name."""
    OUT.mkdir(parents=True, exist_ok=True)
    ...
    # Fallback: existing flutter-apk or previous release APK (rename to 0.0.2)
    candidates = [
