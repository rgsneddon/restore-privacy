# Restore Privacy **0.3.0** — release notes

**Status:** Public package release (catalog + packaging).  
**Tag:** `0.3.0`  
**URL:** https://github.com/rgsneddon/restore-privacy/releases/tag/0.3.0

Catalog advances from **0.2.9** to **0.3.0**.
...
| Asset | Notes |
|-------|--------|
| `restore-privacy-client-0.3.0-windows-x64-setup.exe` | Setup installer |
| `restore-privacy-client-0.3.0-android.apk` | Flutter APK |
| `restore-privacy-client-0.3.0-linux-x64.tar.gz` | Installer package |
| `restore-privacy-client-0.3.0-macos.zip` | **Developer ID signed + notarized** when rebuilt on macOS; may stage prior signed tree when tooling absent |
| `restore-privacy-client-0.3.0-ios.zip` | **Team-signed sideload** when rebuilt; may stage prior when tooling absent |
...
Install **0.3.0** from this GitHub Release ([0.3.0](https://github.com/rgsneddon/restore-privacy/releases/tag/0.3.0)) or the VPN APP Shop download catalog (catalog **v0.3.0**, paid buttons fulfil the same assets). Accept the end-user licence on first Connect.
