#!/usr/bin/env python3
"""Build / stage Restore Privacy client packages for release 0.3.5.

**0.3.5** node-only zram+LUKS2 catalog. Apple packages are Flutter-built then
DevID/notarized (macOS) / Team-signed (iOS). Linux is rebuilt via package_linux.
Windows SFX is repacked with VERSION/title pins rewritten to 0.3.5 when a full
...
- Windows / Android / Linux: prefer local 0.3.4 assets renamed to 0.3.5 filenames
...
def stage_windows_exe() -> Path:
    ...
    # Prefer already-rebuilt 0.3.5 SFX if present with correct pin
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--with-carry-forward",
        action="store_true",
        help="Also stage Windows/Android/Linux from 0.3.4 under 0.3.5 names (default if not apple-only)",
    )
    ...
    manifest = {
        ...
        "notes": (
            "0.3.5: live catalog Pay buttons + paid macOS fulfilment pin; residual/client carry from 0.3.4; connect pins production node Developer ID package seal + iOS Team-signed sideload; "
