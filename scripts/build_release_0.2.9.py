#!/usr/bin/env python3
"""Build / stage Restore Privacy client packages for release 0.2.9.

Public GitHub Release **0.2.9** ships **Developer ID–notarized** macOS and
...
def stage_macos_zip() -> Path:
    ...
    try:
        ...
        print(f"macos local package skipped ({exc}); staging prior…", file=sys.stderr)
    prior = f"restore-privacy-client-{PRIOR_TAG}-macos.zip"
    local = ROOT / "releases" / PRIOR_TAG / prior
    if local.is_file() and local.stat().st_size > 1_000_000:
        shutil.copy2(local, dest)
        print(f"macos staged from local prior: {local}")
        return dest
...
def stage_ios_zip() -> Path:
    ...
    try:
        ...
        print(f"ios local package skipped ({exc}); staging prior…", file=sys.stderr)
    prior = f"restore-privacy-client-{PRIOR_TAG}-ios.zip"
    local = ROOT / "releases" / PRIOR_TAG / prior
    if local.is_file() and local.stat().st_size > 1_000_000:
        shutil.copy2(local, dest)
        print(f"ios staged from local prior: {local}")
        return dest
    fetch_prior_asset(prior, dest)
    return dest
...
def main() -> int:
    ...
    try:
        ...
        if rc == 0 and linux_path.is_file():
            ...
            raise RuntimeError("package_linux failed or missing output")
    except Exception as exc:  # noqa: BLE001
        print(f"linux package skipped ({exc}); staging prior…", file=sys.stderr)
        prior = f"restore-privacy-client-{PRIOR_TAG}-linux-x64.tar.gz"
        local = ROOT / "releases" / PRIOR_TAG / prior
        if local.is_file() and local.stat().st_size > 1_000_000:
            shutil.copy2(local, linux_path)
        else:
            fetch_prior_asset(prior, linux_path)
        artifacts[LINUX_TGZ_NAME] = sha256_file(linux_path)
        print(f"linux:   {linux_path} ({linux_path.stat().st_size} bytes)")
    ...
    manifest = {
        ...
        "notes": (
            "0.2.9: macOS hide-to-tray after full-tunnel Connect; residual Team NE path; "
            "security audit 4h refresh; catalog jump from 0.2.3. "
            "0.2.3: Settings transparency, licence gate, native pad/obfs/PFS."
        ),
    }
