**0.3.1** — Fixed macOS public package seal (Developer ID + notarized) and iOS Team-signed sideload. Non-Apple platforms carry-forward from 0.3.0.

# Restore Privacy **0.3.1** — release notes

**Status:** Public package release (catalog + packaging).  
**Tag:** `0.3.1`  
**URL:** https://github.com/rgsneddon/restore-privacy/releases/tag/0.3.1

Catalog advances from **0.2.9** to **0.3.1**.
...
| Asset | Notes |
|-------|--------|
| `restore-privacy-client-0.3.1-windows-x64-setup.exe` | Setup installer |
| `restore-privacy-client-0.3.1-android.apk` | Flutter APK |
| `restore-privacy-client-0.3.1-linux-x64.tar.gz` | Installer package |
| `restore-privacy-client-0.3.1-macos.zip` | **Developer ID signed + notarized** when rebuilt on macOS; may stage prior signed tree when tooling absent |
| `restore-privacy-client-0.3.1-ios.zip` | **Team-signed sideload** when rebuilt; may stage prior when tooling absent |
