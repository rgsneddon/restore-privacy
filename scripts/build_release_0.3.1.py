#!/usr/bin/env python3
"""Build / stage Restore Privacy client packages for release 0.3.1.

**0.3.1** fixes Apple public packages (macOS Developer ID + notarized; iOS
Team-signed sideload). Windows / Android / Linux are carry-forward renames
from the prior catalog pin (**0.3.0**) unless rebuilt on this host.
...
- Windows / Android / Linux: prefer local 0.3.0 assets renamed to 0.3.1 filenames
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--with-carry-forward",
        action="store_true",
        help="Also stage Windows/Android/Linux from 0.3.0 under 0.3.1 names (default if not apple-only)",
    )
    ...
    manifest = {
        ...
        "notes": (
            "0.3.1: fixed macOS Developer ID package seal + iOS Team-signed sideload; "
            "public host without NE, Packet Tunnel appex with packet-tunnel-provider; "
            "non-Apple platforms carry-forward from 0.3.0 under new catalog filenames."
        ),
    }
