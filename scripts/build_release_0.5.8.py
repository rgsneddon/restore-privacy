#!/usr/bin/env python3
"""Build / stage Restore Privacy client packages for release 0.5.8.

**0.5.8** multi-hop residual catalog. Apple ship path (``--apple-only``) always:
...
**Windows multihop PE** must be built on Windows x64::
  ...
  # or:  python scripts/build_release_0.5.8.py --windows-only
...
Handoff: ``client/windows/WINDOWS_HANDOFF_0.5.8.md``
"""
...
def stage_windows_exe(*, force_rebuild: bool = False) -> Path:
    ...
    if force_rebuild or sys.platform.startswith("win"):
        try:
            ...
            if force_rebuild:
                raise
    # Prefer already-rebuilt 0.5.8 SFX if present with correct pin
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.windows_only:
        ...
        manifest = {
            ...
            "notes": "0.5.8 Windows multihop residual PE + window_foreground (build_windows_multihop / --windows-only)",
        }
        if man_path.is_file():
            try:
    ...
    manifest = {
        ...
        "notes": (
            "0.5.8: residual IPv4/IPv6 Settings; macOS DevID notarize + Team residual NE "
