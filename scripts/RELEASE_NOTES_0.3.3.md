**0.3.3** — Removed flyclient connect fast path; always HELLO on Connect.

**0.3.3** — Catalog rebuild: Apple IPv6 kill-switch removed; all platforms pin production node 82.221.101.241:44044; packages ship node_elgamal.pub only.

**0.3.3** — Fixed macOS public package seal (Developer ID + notarized) and iOS Team-signed sideload. Non-Apple platforms carry-forward from 0.3.0.
...
Catalog advances from **0.2.9** to **0.3.3**.
...
| Asset | Notes |
|-------|--------|
| `restore-privacy-client-0.3.3-windows-x64-setup.exe` | Setup installer |
| `restore-privacy-client-0.3.3-android.apk` | Flutter APK |
| `restore-privacy-client-0.3.3-linux-x64.tar.gz` | Installer package |
| `restore-privacy-client-0.3.3-macos.zip` | **Developer ID signed + notarized** when rebuilt on macOS; may stage prior signed tree when tooling absent |
| `restore-privacy-client-0.3.3-ios.zip` | **Team-signed sideload** when rebuilt; may stage prior when tooling absent |
...
Install **0.3.3** from this GitHub Release ([0.3.3](https://github.com/rgsneddon/restore-privacy/releases/tag/0.3.3)) or the status page download catalog (catalog **v0.3.3**, paid buttons fulfil the same assets). Accept the end-user licence on first Connect.
