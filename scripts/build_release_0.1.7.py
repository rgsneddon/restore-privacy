#!/usr/bin/env python3
"""Build / stage Restore Privacy client packages for release 0.1.7.
...
def write_version_files() -> None:
    ...
    if inst.is_file():
        ...
        t2 = t
        for old in ("0.1.6", "0.1.5", "0.1.4", "0.1.3", "0.1.2", "0.1.1", "0.1.0", "0.0.9"):
...
def main() -> int:
    ...
    manifest = {
        ...
        "notes": (
            "0.1.7: tray logo+status, mojibake-free UI, same-version install speed. 0.1.6: tray Connect/Disconnect status icon+tooltip; residual honesty; Settings; autoclose installer. 0.1.3: per-device Ed25519 keys; no shared client priv; full tunnel teardown on app close (Windows + Android); "
