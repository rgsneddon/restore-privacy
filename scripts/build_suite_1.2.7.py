#!/usr/bin/env python3
"""Build Restore Privacy residual VPN monopin 1.2.7 installers and stage for Helsinki.
...
Usage::
  ...
  python3 scripts/build_suite_1.2.7.py
  python3 scripts/build_suite_1.2.7.py --skip-build   # stage/copy only
  python3 scripts/build_suite_1.2.7.py --host-paid     # stage + Helsinki upload
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Windows / Linux: no native rebuild on this Darwin host — carry 1.2.6
    # sealed artifacts forward under the 1.2.7 catalog names.
    if not only:
        for plat in ("windows", "linux"):
            if plat not in built:
