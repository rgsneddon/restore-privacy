...
| Tag | Script |
|-----|--------|
| **0.3.8** | `scripts/build_release_0.3.8.py` |
| **0.3.8 Windows multihop PE** | `scripts/build_windows_multihop.py` / `scripts/build_windows_multihop.bat` (Windows x64 only; handoff `client/windows/WINDOWS_HANDOFF_0.3.8.md`) |
| **0.3.8 Apple handoff** | `client_app/APPLE_HANDOFF_0.3.8.md` |
...
Product node: **82.221.101.241:44044**. See `scripts/RELEASE_NOTES_0.3.8.md`.

**0.3.8 highlights:** catalog monopin **0.3.8**; subscription keygen unlock; multi-hop residual when enabled; Apple packages via Mac handoff `client_app/APPLE_HANDOFF_0.3.8.md`.
...
python scripts/build_release_0.3.8.py --apple-only
# Confirm releases/0.3.8/ has macos (+ ios if built) zip(s)
# Full catalog: python scripts/build_release_0.3.8.py
# gh release create 0.3.8 with those files (operator)
```
