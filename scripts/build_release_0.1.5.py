#!/usr/bin/env python3
"""Build / stage Restore Privacy client packages for release 0.1.5.
...
def write_version_files() -> None:
    ...
    if inst.is_file():
        ...
        t2 = t
        for old in ("0.1.4", "0.1.3", "0.1.2", "0.1.1", "0.1.0", "0.0.9"):
...
def sign_and_notarize_macos(app: Path, dest_zip: Path) -> None:
    ...
    This is what prevents Gatekeeper "Apple could not verify â€¦ free of malware"
...
def fetch_prior_asset(name: str, dest: Path) -> Path:
    ...
    try:
        ...
        print(f"urllib fetch failed ({exc}); trying gh…", file=sys.stderr)
    cmd = [
        "gh",
        "release",
        "download",
...
def stage_windows_exe() -> Path:
    ...
    try:
        ...
        print(f"Windows rebuild failed ({exc}); staging prior…", file=sys.stderr)
    local_candidates = [
...
def main() -> int:
    ...
    manifest = {
        ...
        "notes": (
            "0.1.3: per-device Ed25519 keys; no shared client priv; full tunnel teardown on app close (Windows + Android); "
            "Packet Tunnel quit/stop; catalog + privacy docs aligned."
        ),
    }
    man_path = OUT / "SHA256SUMS.json"
    man_path.write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
    # Also write manifest.json for parity with older recipes
