#!/usr/bin/env python3
"""Build Restore Privacy residual VPN monopin 1.2.6 installers and stage for Helsinki.
...
(residual VPN client shell). **Windows** and **linux** are **not** restamped
here: this Mac does not Authenticode-sign a Windows PE, and must not overwrite
Helsinki ``paid_assets/1.2.5/`` Windows. Linux is an honest skip unless a
native rebuild is produced on a Linux agent.

Usage::

  python3 scripts/build_suite_1.2.6.py
  python3 scripts/build_suite_1.2.6.py --skip-build   # stage/copy only
  python3 scripts/build_suite_1.2.6.py --host-paid     # stage + Helsinki upload
"""
...
def inject_ios_residual_pubs(runner: Path) -> Path:
    ...
    Live catalog: DE ``de_node_elgamal.pub``, SG ``sg_node_elgamal.pub``,
    multihop ``exit_node_elgamal.pub`` (and legacy IS file). **Does not**
    inject retired ``us_node_elgamal.pub``. Fail-closed if entry pin missing. Uses
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Windows: never staged on this Mac — do not restamp Helsinki 1.2.5 PE as 1.2.6.
    # Linux: honest skip (no native rebuild on Darwin; do not restamp 1.2.5 tarball).
    print(
        "windows: skip — native PE is not staged on this Mac; "
        "Helsinki paid_assets/1.2.5 Windows stays; Windows machine rebuilds 1.2.6",
        flush=True,
    )
    print(
        "linux: skip — no native Linux rebuild on this Darwin host; "
        "do not restamp a prior tarball as 1.2.6",
        flush=True,
    )
    # 1.2.6: never restamp a prior pin as this pin. Missing native platforms
    # are honest skips (Windows/Linux always; macOS/iOS/Android only if build failed).
    if not only:
        print(
            "carry-forward disabled for 1.2.6 — native artifacts only "
            f"(built so far: {sorted(built)})",
            flush=True,
        )
