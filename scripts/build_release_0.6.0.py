#!/usr/bin/env python3
"""Build / stage Restore Privacy client packages for release 0.6.0.

**0.6.0** multi-hop residual catalog. Apple ship path (``--apple-only``) always:
...
**Windows multihop PE** must be built on Windows x64::
  ...
  # or:  python scripts/build_release_0.6.0.py --windows-only
...
Handoff: ``client/windows/WINDOWS_HANDOFF_0.6.0.md``
"""
...
def stage_windows_exe(*, force_rebuild: bool = False) -> Path:
    ...
    if force_rebuild or sys.platform.startswith("win"):
        try:
            ...
            if force_rebuild:
                raise
    # Prefer already-rebuilt 0.6.0 SFX if present with correct pin
...
def stage_ios_zip() -> Path:
    ...
    stale Runner.app (e.g. 0.5.9) cannot be published as the current pin.
    """
    dest = OUT / IOS_ZIP_NAME
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--no-apple",
        action="store_true",
        help="Skip macOS/iOS (Windows host ship path: Win + Linux + Android only)",
    )
    ap.add_argument(
        "--with-carry-forward",
        action="store_true",
    ...
    if args.windows_only:
        ...
        manifest = {
            ...
            "notes": "0.6.0 Windows multihop residual PE + window_foreground (build_windows_multihop / --windows-only)",
        }
        if man_path.is_file():
            try:
    ...
    # Default on non-Darwin: ship Win/Linux/Android; Mac picks up Apple via breadcrumbs
    skip_apple = bool(args.no_apple) or (
        not args.apple_only and not sys.platform.startswith("darwin")
    )
    if args.apple_only:
        skip_apple = False
    ...
    if not skip_apple:
        ...
        print(
            f"apple: skipped on this host (Mac breadcrumbs for monopin {VERSION})",
            flush=True,
        )

    do_carry = args.with_carry_forward or not args.apple_only
    if do_carry:
        win = stage_windows_exe(force_rebuild=sys.platform.startswith("win"))
        artifacts[WINDOWS_EXE_NAME] = sha256_file(win)
    ...
    required: list[str] = []
    if not skip_apple:
        required.extend([MACOS_ZIP_NAME, IOS_ZIP_NAME])
    if do_carry:
    ...
    manifest = {
        ...
        "notes": (
            "0.6.0: residual IPv4/IPv6 Settings; macOS DevID notarize + Team residual NE "
