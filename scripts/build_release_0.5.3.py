#!/usr/bin/env python3
"""Build / stage Restore Privacy client packages for release 0.5.3.

**0.5.3** multi-hop residual catalog. Apple packages are Flutter-built then
...
**Windows multihop PE** must be built on Windows x64::
  ...
  # or:  python scripts/build_release_0.5.3.py --windows-only
...
Handoff: ``client/windows/WINDOWS_HANDOFF_0.5.3.md``
"""
...
def stage_windows_exe(*, force_rebuild: bool = False) -> Path:
    ...
    if force_rebuild or sys.platform.startswith("win"):
        try:
            ...
            if force_rebuild:
                raise
    # Prefer already-rebuilt 0.5.3 SFX if present with correct pin
...
def main(argv: list[str] | None = None) -> int:
    ...
    manifest = {
        ...
        "notes": (
            "0.5.3: Windows/Linux/Android ship from this host; "
