#!/usr/bin/env python3
"""Build / stage Restore Privacy client packages for release 0.4.5.

**0.4.5** multi-hop residual catalog. Apple packages are Flutter-built then
...
**Windows multihop PE** must be built on Windows x64::
  ...
  # or:  python scripts/build_release_0.4.5.py --windows-only
...
Handoff: ``client/windows/WINDOWS_HANDOFF_0.4.5.md``
"""
...
def stage_windows_exe(*, force_rebuild: bool = False) -> Path:
    ...
    if force_rebuild or sys.platform.startswith("win"):
        try:
            ...
            if force_rebuild:
                raise
    # Prefer already-rebuilt 0.4.5 SFX if present with correct pin
...
def stage_macos_zip() -> Path:
    ...
    try:
        ...
        print(
            ...
            f"carry-forward prior zip → {dest.name} (Mac must rebuild/sign for real 0.4.5)",
            file=sys.stderr,
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--with-carry-forward",
        action="store_true",
        help="Also stage Windows/Android/Linux under 0.4.5 names (default if not apple/windows-only)",
    )
    ...
    if args.windows_only:
        ...
        manifest = {
            ...
            "notes": "0.4.5 Windows multihop residual PE (build_windows_multihop / --windows-only)",
        }
    ...
    manifest = {
        ...
        "notes": (
            "0.4.5: live catalog Pay buttons + paid macOS fulfilment pin; residual/client carry from 0.3.4; connect pins production node Developer ID package seal + iOS Team-signed sideload; "
