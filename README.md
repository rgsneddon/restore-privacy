...
| | |
|--|--|
| **Get the app** | [Paid downloads](https://restoreprivacy.online/) — catalog **v0.4.6** (£2.45/month or £27.93/year per platform) |
...
- Connect uses the standard **HELLO** residual path (**flyclient** fast-path removed in catalog **v0.4.6**).
...
- Multi-hop residual is **opt-in** (`RPT_MULTIHOP_ENABLED=1`): exit is a **random non-entry** catalog peer (RO or DE when entry is IS). Default is single-hop on the chosen entry. **residual-via-exit** routing is implemented (not hop-list-only).
- Status site shows a **live preferred-entry clear timer** (~7 days). Fleet wipe is **sequential** (IS → RO → DE, one peer at a time) — never concurrent multi-node wipe.
...
**Current catalog (v0.4.6):** the homepage **Download client** box has device/platform and plan selection — **Monthly VPN plan** £2.45 / **Yearly VPN plan** £27.93 (5% off) — plus **Buy now**, which opens Stripe Checkout. You can also open the plan page at **`/pay`**. Card payment uses Stripe’s hosted checkout (Dashboard branding only — not this site’s full CSS). Email delivers **keygen + PPI + download link** (**USE THIS KEYGEN TO UNLOCK RESTORE PRIVACY**).
...
| Platform | Package |
|----------|---------|
| Windows | `restore-privacy-client-0.4.6-windows-x64-setup.exe` |
| Android | `restore-privacy-client-0.4.6-android.apk` |
| macOS | `restore-privacy-client-0.4.6-macos.zip` *(Developer ID signed + notarized)* |
| iOS | `restore-privacy-client-0.4.6-ios.zip` *(Team-signed sideload)* |
| Ubuntu / Linux | `restore-privacy-client-0.4.6-linux-x64.tar.gz` |
| Browser (Chromium MV3) | `restore-privacy-browser-extension-0.4.6.zip` — browser proxy only, not OS residual TUN |

### Browser extension (Chromium MV3)

1. Get **`restore-privacy-browser-extension-0.4.6.zip`** from the [0.4.6 release assets](https://github.com/rgsneddon/restore-privacy/releases/tag/0.4.6) (or build from `browser_extension/`).
...
1. On the [status downloads page](https://restoreprivacy.online/), choose **Monthly £2.45** or **Yearly** for **Windows** and download **`restore-privacy-client-0.4.6-windows-x64-setup.exe`** (one-time link after payment).
...
1. On the [status downloads page](https://restoreprivacy.online/), choose **Monthly £2.45** or **Yearly** for **Android** and download **`restore-privacy-client-0.4.6-android.apk`** (one-time link after payment).
...
1. On the [status downloads page](https://restoreprivacy.online/), choose **Monthly £2.45** or **Yearly** for **Linux** and download **`restore-privacy-client-0.4.6-linux-x64.tar.gz`** (one-time link after payment).
2. Unpack and run the bundled installer:

   ```bash
   tar xzf restore-privacy-client-0.4.6-linux-x64.tar.gz
...
Published **v0.4.6** macOS builds are **Developer ID signed and notarized**.

1. On the [status downloads page](https://restoreprivacy.online/), choose **Monthly £2.45** or **Yearly** for **macOS** and download **`restore-privacy-client-0.4.6-macos.zip`** (one-time link after payment).
...
4. Residual public IP only changes when the Packet Tunnel is **active**. Host-only HELLO is **diagnostic** only. Residual public-IP via Packet Tunnel on a developer Mac still needs **Team residual re-sign** (`scripts/sign_macos_residual_team.py`) — the public Developer ID zip alone is not full host-NE residual (see `client_app/APPLE_HANDOFF_0.4.6.md`). **Disconnect** / **Quit** stops the system VPN.
5. Failsafe: run **`Restore Internet.command`** in the package (or follow VPN Settings cleanup) — see warning below.

### iOS

Published **v0.4.6** iOS packages are **Team-signed sideload** zips (not App Store).

1. On the [status downloads page](https://restoreprivacy.online/), choose **Monthly £2.45** or **Yearly** for **iOS** and download **`restore-privacy-client-0.4.6-ios.zip`** (one-time link after payment).
...
- **Monthly £2.45** and **Yearly** pay controls per platform (Windows, Android, macOS, iOS, Linux) — catalog **v0.4.6**
...
- **VPS / provider IP metadata** — catalog peers include **FlokiNET** (IS/RO) and the **DE** residual host; host public **no invasive logs** stance is not a forensic audit. Other providers (CDN/status, home ISP, destinations) may still log. Node OS compromise remains a residual risk.
...
**Release scripts:** `scripts/build_release_0.4.6.py`. **Windows multihop PE** (x64 only): `scripts/build_windows_multihop.py` / `scripts\build_windows_multihop.bat` — handoff [`client/windows/WINDOWS_HANDOFF_0.4.6.md`](client/windows/WINDOWS_HANDOFF_0.4.6.md). Release notes: [`scripts/RELEASE_NOTES_0.4.6.md`](scripts/RELEASE_NOTES_0.4.6.md). Catalog **0.4.6** Windows paid package is native PE when rebuilt; prior PE-carry-forward until rebuilt on Windows x64; multihop residual-via-exit remains opt-in (`RPT_MULTIHOP_ENABLED=1` / Settings multi-hop).
...
**Node wipe reinstall (sequential fleet):** [docs/NODE_WIPE_REINSTALL.md](docs/NODE_WIPE_REINSTALL.md) — weekly wipe is **one peer at a time** (IS → RO → DE) with mandatory full selfhost reinstall; never concurrent multi-node wipe.
