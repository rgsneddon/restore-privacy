...
| | |
|--|--|
| **Get the app** | [Paid downloads](https://restoreprivacy.online/) — catalog **v0.5.4** (£2.45/month or £27.93/year per platform) |
...
- Connect uses the standard **HELLO** residual path (**flyclient** fast-path removed in catalog **v0.5.4**).
...
**Current catalog (v0.5.4):** the homepage **Download client** box has device/platform and plan selection — **Monthly VPN plan** £2.45 / **Yearly VPN plan** £27.93 (5% off) — plus **Buy now**, which opens Stripe Checkout. You can also open the plan page at **`/pay`**. Card payment uses Stripe’s hosted checkout (Dashboard branding only — not this site’s full CSS). Email delivers **keygen + PPI + download link** (**USE THIS KEYGEN TO UNLOCK RESTORE PRIVACY**).
...
| Platform | Package |
|----------|---------|
| Windows | `restore-privacy-client-0.5.4-windows-x64-setup.exe` *(**native** multihop PE; includes window_foreground)* |
...
1. Get **`restore-privacy-browser-extension-0.5.4.zip`** from the [0.5.1 release assets](https://github.com/rgsneddon/restore-privacy/releases/tag/0.5.4) (or build from `browser_extension/`).
...
1. On the [status downloads page](https://restoreprivacy.online/), choose **Monthly £2.45** or **Yearly** for **Windows** and download **`restore-privacy-client-0.5.4-windows-x64-setup.exe`** (one-time link after payment).
...
1. On the [status downloads page](https://restoreprivacy.online/), choose **Monthly £2.45** or **Yearly** for **Android** and download **`restore-privacy-client-0.5.4-android.apk`** (one-time link after payment).
...
1. On the [status downloads page](https://restoreprivacy.online/), choose **Monthly £2.45** or **Yearly** for **Linux** and download **`restore-privacy-client-0.5.4-linux-x64.tar.gz`** (one-time link after payment).
2. Unpack and run the bundled installer:

   ```bash
   tar xzf restore-privacy-client-0.5.4-linux-x64.tar.gz
...
**Monopin 0.5.4 macOS is not on the paid store yet.** A Mac-native Developer ID + notarized seal is required (`client_app/APPLE_HANDOFF_0.5.4.md`, Helsinki breadcrumbs vault). Until that seal is published under `paid_assets/0.5.4/`, there is **no** live download — do not treat catalog filename alone as a sealed package.
...
4. Residual public IP only changes when the Packet Tunnel is **active**. Host-only HELLO is **diagnostic** only. **Disconnect** / **Quit** stops the system VPN. See `client_app/APPLE_HANDOFF_0.5.4.md`.
...
- **Monthly £2.45** and **Yearly** pay controls per platform (Windows, Android, macOS, iOS, Linux) — catalog **v0.5.4**
...
**Product ship (v0.5.4):** Paid installers on **[status downloads](https://restoreprivacy.online/)**. **Windows** and **Linux** are **native 0.5.4** rebuilds on this pin; **Android** is honest residual-wire CF from **0.5.0** (or **0.4.10** wire); **macOS** is **native monopin 0.5.4** (`CFBundleShortVersionString` **0.5.1**); **iOS** is native monopin sideload when Mac-built — see `APPLE_HANDOFF_0.5.4.md`. Source repo is private. Catalog residual peers: **IS** `82.221.101.241:44044` (selectable; default residual is United States), **RO** `185.146.232.107:44044`, **US** `5.161.242.85:44044` — user-selectable entry; host public **no invasive logs** stance where published (see privacy policy).
...
**Release scripts:** `scripts/build_release_0.5.4.py`. **Windows multihop PE** (x64 only): `scripts/build_windows_multihop.py` / `scripts\build_windows_multihop.bat` — handoff [`client/windows/WINDOWS_HANDOFF_0.5.4.md`](client/windows/WINDOWS_HANDOFF_0.5.4.md). Release notes: [`scripts/RELEASE_NOTES_0.5.4.md`](scripts/RELEASE_NOTES_0.5.4.md). Catalog **0.5.4** **Windows** paid package on this ship is **native PE rebuild**; **Linux** is **native rebuild**; **Android** is CF residual-wire; **macOS** native monopin CFBundle **0.5.4**; **iOS** native when Mac-built; multihop residual-via-exit remains opt-in (`RPT_MULTIHOP_ENABLED=1` / Settings multi-hop).
...
python scripts/build_release_0.5.4.py
```
