...
| | |
|--|--|
| **Get the app** | [Download v0.2.0](https://github.com/rgsneddon/restore-privacy/releases/tag/0.2.0)  |  [Status & downloads](https://restore-privacy-status.onrender.com/) |
...
Download packages from the **[0.2.0 release](https://github.com/rgsneddon/restore-privacy/releases/tag/0.2.0)**  
or use the buttons on https://restore-privacy-status.onrender.com/

| Platform | Package |
|----------|---------|
| Windows | `restore-privacy-client-0.2.0-windows-x64-setup.exe` |
| Android | `restore-privacy-client-0.2.0-android.apk` |
| macOS | `restore-privacy-client-0.2.0-macos.zip` *(prep zip / sign on Mac  -  see below)* |
| iOS | `restore-privacy-client-0.2.0-ios.zip` *(prep zip / sign on Mac  -  see below)* |
| Ubuntu / Linux | `restore-privacy-client-0.2.0-linux-x64.tar.gz` *(installer package; crypto deps baked in)* |

### Windows

1. Download the **Windows installer (.exe)** from the release or status page.
2. Run **`restore-privacy-client-0.2.0-windows-x64-setup.exe`**.  
...
4. Optional: open **Settings** and enable **Run at device startup** and/or **Autoconnect on launch** for seamless power-up (both default **off**).
...
1. Download **`restore-privacy-client-0.2.0-linux-x64.tar.gz`** (installer package) from the release or status page.
2. Unpack and run the **bundled installer** (installs app Python deps **from wheels inside the archive**  -  no network `pip install cryptography`):
   ```bash
   tar xzf restore-privacy-client-0.2.0-linux-x64.tar.gz
   cd restore-privacy-0.2.0-linux
   bash install.sh
   ```
...
Release zips for **0.2.0** are **prep packages** for sideload / further signing  -  **Mac work required**. Residual public IP does **not** change until Packet Tunnel / Network Extension is signed and active; host-side HELLO alone is diagnostic only. **Team signing and notarization must be done on a Mac.**

1. Download **`restore-privacy-client-0.2.0-macos.zip`** or **`restore-privacy-client-0.2.0-ios.zip`**, **or** clone this repo and open `client_app/` in Xcode / Flutter on macOS.
2. Developer checklist:  
   ...
   - Mac handoff notes: [`client_app/APPLE_HANDOFF_0.2.0.md`](client_app/APPLE_HANDOFF_0.2.0.md) (created with the release)
...
- **Download** buttons for Windows, Android, macOS, iOS, and Linux (catalog v0.2.0)  
...
**Secrets discipline:** Never commit or force-add `secrets/` (gitignored). Public packages must never include `node_elgamal.priv` or a shared `client_ed25519.priv`. Release scripts run `_assert_no_priv` / strip inject gates  -  keep those on every tag. VPS/CDN operators may still log IP-level traffic under **their** policies (outside product no-log; see `PRIVACY_POLICY.md` section 4).

**0.2.0 release:** Production node **82.221.101.241:44044**. UK geo gate **removed**; tunnel DNS defaults to node **10.88.0.1**; IPv6 leak honesty. See [scripts/RELEASE_NOTES_0.2.0.md](scripts/RELEASE_NOTES_0.2.0.md). Upgrade from 0.1.x so clients leave the old VPS IP.

**Tunnel DNS / host privacy:** [node/install_dns.sh](node/install_dns.sh), [node/install_host_privacy.sh](node/install_host_privacy.sh). Connect does not phone home to third-party geo APIs.
