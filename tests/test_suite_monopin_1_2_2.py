"""Suite monopin 1.2.2 catalog + handoff pins (shipped sources)."""
...
class TestSuiteMonopin122(unittest.TestCase):
    def test_version_file_is_1_2_2(self) -> None:
    ...
    def test_downloads_catalog_pin(self) -> None:
        ...
        self.assertEqual(RELEASE_VERSION, "1.2.2")
        self.assertEqual(RELEASE_TAG, "1.2.2")
        pkgs = list_catalog_platform_packages()
        self.assertEqual(len(pkgs), 5)
        for p in pkgs:
            self.assertEqual(p["version"], "1.2.2")
            self.assertIn("1.2.2", p["filename"])
            self.assertTrue(
                p["filename"].startswith("restore-privacy-client-1.2.2-")
            )
    ...
    def test_suite_version_dart_pin(self) -> None:
        ...
        self.assertIn("kSuiteVersion = '1.2.2'", dart)

    def test_pubspec_pin(self) -> None:
        pub = (ROOT / "client_app" / "pubspec.yaml").read_text(encoding="utf-8")
        self.assertIn("version: 1.2.2+", pub)

    def test_rpt_config_product_version(self) -> None:
        cfg = (ROOT / "client_app" / "lib" / "rpt_config.dart").read_text(
            encoding="utf-8"
        )
        self.assertIn("productVersion = '1.2.2'", cfg)

    def test_windows_handoff_1_2_2_exists(self) -> None:
        h = ROOT / "client" / "windows" / "WINDOWS_HANDOFF_1.2.2.md"
        self.assertTrue(h.is_file(), f"missing {h}")
        text = h.read_text(encoding="utf-8")
        self.assertIn("1.2.2", text)
        self.assertIn(
            "restore-privacy-client-1.2.2-windows-x64-setup.exe", text
        )
    ...
    def test_build_suite_1_2_2_exists(self) -> None:
        script = ROOT / "scripts" / "build_suite_1.2.2.py"
        self.assertTrue(script.is_file())
        text = script.read_text(encoding="utf-8")
        self.assertIn('VERSION = "1.2.2"', text)
        self.assertIn("1.2.1", text)  # carry-forward prior
    ...
    def test_android_lock_recovery_in_tree(self) -> None:
        """1.2.2 product path includes residual lock/desired re-HELLO honesty."""
        svc = (
            ROOT
