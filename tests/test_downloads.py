"""Tests for shipped status-page download catalog (release 0.0.8)."""
...
class TestDownloadCatalog(unittest.TestCase):
    def test_version_is_0_0_2(self):
        self.assertEqual(RELEASE_VERSION, "0.0.8")
        self.assertEqual(RELEASE_TAG, "0.0.8")
    ...
    def test_public_assets_are_exe_and_apk_only(self):
        ...
        self.assertIn("0.0.8", WINDOWS_EXE_FILENAME)
        self.assertIn("0.0.8", ANDROID_APK_FILENAME)

    def test_available_downloads_have_https_github_release_urls(self):
        assets = available_downloads()
        for a in assets:
            self.assertTrue(a.url.startswith("https://"))
            self.assertIn("/releases/download/0.0.8/", a.url)
            self.assertIn(a.filename, a.url)
            self.assertIn("0.0.8", a.filename)
    ...
    def test_render_download_section_uses_real_urls(self):
        ...
        self.assertIn("Download client v0.0.8", html)
    ...
    def test_status_page_html_includes_downloads(self):
        ...
        self.assertIn("Download client v0.0.8", page)
        self.assertIn("releases/download/0.0.8/", page)
...
class TestInstallerPackagingRecipe(unittest.TestCase):
    """Structural: 0.0.8 build recipe produces the advertised setup .exe name."""
