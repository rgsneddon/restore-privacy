"""Tests for shipped status-page download catalog (release 0.0.4)."""
...
class TestDownloadCatalog(unittest.TestCase):
    def test_version_is_0_0_2(self):
        self.assertEqual(RELEASE_VERSION, "0.0.4")
        self.assertEqual(RELEASE_TAG, "0.0.4")
    ...
    def test_public_assets_are_exe_and_apk_only(self):
        ...
        self.assertIn("0.0.4", WINDOWS_EXE_FILENAME)
        self.assertIn("0.0.4", ANDROID_APK_FILENAME)

    def test_available_downloads_have_https_github_release_urls(self):
        assets = available_downloads()
        for a in assets:
            self.assertTrue(a.url.startswith("https://"))
            self.assertIn("/releases/download/0.0.4/", a.url)
            self.assertIn(a.filename, a.url)
            self.assertIn("0.0.4", a.filename)
    ...
    def test_render_download_section_uses_real_urls(self):
        ...
        self.assertIn("Download client v0.0.4", html)
    ...
    def test_status_page_html_includes_downloads(self):
        ...
        self.assertIn("Download client v0.0.4", page)
        self.assertIn("releases/download/0.0.4/", page)
...
class TestInstallerPackagingRecipe(unittest.TestCase):
    """Structural: 0.0.4 build recipe produces the advertised setup .exe name."""

    def test_build_script_wires_exe_and_apk_names(self):
        script = (ROOT / "scripts" / "build_release_0.0.4.py").read_text(encoding="utf-8")
        self.assertIn('VERSION = "0.0.4"', script)
    ...
    def test_installer_module_deploys_and_launches(self):
        ...
        self.assertIn("VERSION = \"0.0.4\"", inst)
