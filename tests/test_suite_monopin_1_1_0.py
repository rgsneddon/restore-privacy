"""Suite monopin 1.1.0 catalog + handoff pins (shipped sources)."""
...
class TestSuiteMonopin110(unittest.TestCase):
    def test_version_file_is_1_1_0(self) -> None:
    ...
    def test_downloads_catalog_pin(self) -> None:
        ...
        self.assertEqual(RELEASE_TAG, "1.1.0")
        ...
        for p in pkgs:
            ...
            self.assertIn("1.1.0", p["filename"])
            self.assertTrue(
                p["filename"].startswith("restore-privacy-client-1.1.0-")
            )
    ...
    def test_suite_version_dart_pin(self) -> None:
        ...
        self.assertIn("kSuiteVersion = '1.1.0'", dart)
        self.assertIn("Restore Privacy Suite v 1.1.0", dart)

    def test_pubspec_pin(self) -> None:
        pub = (ROOT / "client_app" / "pubspec.yaml").read_text(encoding="utf-8")
        self.assertIn("version: 1.1.0+1", pub)

    def test_windows_handoff_1_1_0_exists(self) -> None:
        h = ROOT / "client" / "windows" / "WINDOWS_HANDOFF_1.1.0.md"
        self.assertTrue(h.is_file(), f"missing {h}")
        text = h.read_text(encoding="utf-8")
        self.assertIn("1.1.0", text)
        self.assertIn(
            "restore-privacy-client-1.1.0-windows-x64-setup.exe", text
        )
