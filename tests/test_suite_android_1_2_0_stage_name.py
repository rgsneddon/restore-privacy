"""Catalog monopin 1.2.0 Android package naming (shipped suite build path)."""
...
class TestSuiteAndroid120StageName(unittest.TestCase):
    def test_version_pin_and_android_filename(self) -> None:
        pin = (ROOT / "client" / "VERSION").read_text(encoding="utf-8").strip()
        self.assertEqual(pin, "1.2.0")
        suite = (ROOT / "scripts" / "build_suite_1.2.0.py").read_text(encoding="utf-8")
        m = re.search(r'^VERSION = "([^"]+)"', suite, re.M)
        self.assertIsNotNone(m)
        self.assertEqual(m.group(1), pin)
        expected = f"restore-privacy-client-{pin}-android.apk"
        self.assertIn(f'"android": f"restore-privacy-client-{{VERSION}}-android.apk"', suite)
        # build_android stages to releases/VERSION/NAMES["android"]
        ...
        # If a staged artifact is present (operator tree), enforce monopin name/size floor
