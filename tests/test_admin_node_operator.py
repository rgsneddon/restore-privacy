...
class TestAdminNodeOperatorPage(unittest.TestCase):
    def test_sidebar_nav_and_page_render(self) -> None:
        ...
        # Project work surfaces — suite package push / upload to Helsinki
        self.assertIn('id="admin-node-op-deploy-packages"', page)
        self.assertIn("Push Suite packages", page)
        ...
        # No timed full-page reload (manual refresh only)
        self.assertNotIn('http-equiv="refresh"', page)
        self.assertNotIn("admin-node-op-auto-refresh", page)
        self.assertNotIn("data-auto-refresh-sec", page)
        # Selected node preserved in tab links for manual navigation
        self.assertIn("/admin/node-operator?node=IS", page)
