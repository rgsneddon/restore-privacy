"""Suite monopin 1.0.9 catalog + handoff pins (shipped sources)."""
...
class TestSuiteMonopin109(unittest.TestCase):
    def test_version_file_is_1_0_9(self) -> None:
    ...
    def test_downloads_catalog_pin(self) -> None:
        ...
        self.assertEqual(RELEASE_VERSION, "1.0.9")
        self.assertEqual(RELEASE_TAG, "1.0.9")
        result = assure_current_catalog_packages()
        self.assertTrue(result["ok"], result.get("errors"))
        pkgs = list_catalog_platform_packages()
        self.assertEqual(len(pkgs), 5)
        for p in pkgs:
            self.assertEqual(p["version"], "1.0.9")
            self.assertIn("1.0.9", p["filename"])
            self.assertTrue(
                p["filename"].startswith("restore-privacy-client-1.0.9-")
            )
    ...
    def test_suite_version_dart_pin(self) -> None:
        ...
        self.assertIn("kSuiteVersion = '1.0.9'", dart)
        self.assertIn("Restore Privacy Suite v 1.0.9", dart)

    def test_pubspec_pin(self) -> None:
        pub = (ROOT / "client_app" / "pubspec.yaml").read_text(encoding="utf-8")
        self.assertIn("version: 1.0.9+1", pub)

    def test_windows_handoff_1_0_9_exists(self) -> None:
        h = ROOT / "client" / "windows" / "WINDOWS_HANDOFF_1.0.9.md"
        self.assertTrue(h.is_file(), f"missing {h}")
        text = h.read_text(encoding="utf-8")
        self.assertIn("1.0.9", text)
        self.assertIn(
            "restore-privacy-client-1.0.9-windows-x64-setup.exe", text
        )
