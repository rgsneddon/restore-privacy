        ...
        payload = json.dumps(
            {
                ...
                "data": {
                    "object": {
                        ...
                        "metadata": {
                            "platform": "ios",
                            "filename": "restore-privacy-client-0.4.0-ios.zip",
                            "amount_pence": "999",
                            "currency": "gbp",
                        },
                    }
...
class TestPrivateRepoProxyFulfilment(unittest.TestCase):
    ...
    def test_open_release_asset_github_api_sends_auth_header(self):
        fname = "restore-privacy-client-0.4.0-windows-x64-setup.exe"
        ...
        # Empty search dirs so we do not pick up real releases/0.4.0 on disk
        ...
        def fake_urlopen(req, timeout=None):
            ...
            # RELEASE_TAG pin is 0.4.0 (catalog monopin)
            if "/releases/tags/0.4.0" in url:
                body = json.dumps(
    ...
    def test_http_download_streams_not_redirects_to_github(self):
        ...
        fname = "restore-privacy-client-0.4.0-android.apk"
...
class TestPostPaymentThankYouBuilder(unittest.TestCase):
    ...
    def test_builder_thankyou_admin_and_auto_start(self):
        ...
        self.assertIn("restore-privacy-client-0.4.0-windows-x64-setup.exe", html)
        ...
        with self.assertRaises(ValueError):
            payments.render_post_payment_thankyou_html(
                download_path=(
                    ...
                    "download/0.3.0/restore-privacy-client-0.4.0-windows-x64-setup.exe"
                ),
                filename="x.exe",
                platform="windows",
            )
